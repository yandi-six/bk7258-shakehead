...
static void netstream_wifi_event_cb(void *new_evt);

int netstream_sta_connect(char *oob_ssid, char *connect_key)
{
	int ret = 0;

	bk_wlan_status_register_cb(netstream_wifi_event_cb);
	
	ret = demo_sta_app_init(oob_ssid, connect_key);
	if (ret != 0)
	{
		return ret;
	}
...
static void netstream_wifi_event_cb(void *new_evt)
{
	...
	switch (info.state)
	{
		...
		{
			BK_LOGI(TAG, "WIFI_LINKSTATE_STA_GOT_IP\r\n");
			bk_err_t ret = BK_OK;
			...
			msg.op = AUDIO_PLAYER_WIFI_CONNECTED;
			...
			BK_LOGI(TAG, "send connect msg to audio player \r\n");
		}
		break;
		...
		case WIFI_LINKSTATE_STA_DISCONNECTED:
		{
			...
			msg.op = AUDIO_PLAYER_WIFI_DISCONNECT;
			audio_player_send_msg(msg);
		}
		break;

		case WIFI_LINKSTATE_AP_CONNECTED:
		{
			BK_LOGI(TAG, "WIFI_LINKSTATE_AP_CONNECTED\r\n");
		}
		break;

		case WIFI_LINKSTATE_AP_DISCONNECTED:
		{
			BK_LOGI(TAG, "WIFI_LINKSTATE_AP_DISCONNECTED\r\n");
		}
		break;

		default:
			BK_LOGI(TAG, "WIFI_LINKSTATE %d\r\n", info.state);
			break;
...
static bk_err_t netstream_player_start(const char *net_uri)
{
	...
	extern uint32_t current_song;
	song_time.song_uri_id = current_song;
	BK_LOGI(TAG, "content_length = %x, psram_end_addr = 0x%x, song_length:%ds\r\n", content_length, psram_end_addr, song_time.song_time);
		...
		if (kNoErr == ret) {
			switch (msg.op) {
				...
				case NETSTREAM_PLAYER_STOP:
					netstream_player_stop();
					break;
				case NETSTREAM_PLAYER_RESUME:
					// audio_player_resume();
					break;
				case NETSTREAM_PLAYER_NEXT:
					break;
