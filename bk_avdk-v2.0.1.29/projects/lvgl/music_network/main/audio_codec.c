...
static bk_err_t audio_codec_mp3_decoder_init(void)
{
	...
	if (readBuf == NULL) {
		BK_LOGI(TAG,"readBuf malloc failed!\r\n");
		return BK_FAIL;
	}

	pcmBuf = os_malloc(PCM_SIZE_MAX * 2);
	if (pcmBuf == NULL) {
		BK_LOGI(TAG,"pcmBuf malloc failed!\r\n");
		return BK_FAIL;
	}
	...
	if (audio_player_pause_flag != 1)
	{
		...
		if (offset < 0) {
			...
			if (!aud_player_spk_init_flag) {
				...
				song_rate_tag = 0;
				BK_LOGI(TAG,"set mp3 decode samp_rate: %d, mp3FrameInfo.outputSamps: %d\n\r",  mp3FrameInfo.samprate, mp3FrameInfo.outputSamps);
