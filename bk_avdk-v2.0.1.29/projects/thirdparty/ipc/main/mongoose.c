...
static size_t mg_dns_parse_name_depth(const uint8_t *s, size_t len, size_t ofs,
  ...
  while (ofs + i + 1 < len) {
    ...
    if (j + n < tolen) os_memcpy(&to[j], &s[ofs + i + 1], n);
    j += n;
    i += n + 1;
...
bool mg_dns_parse(const uint8_t *buf, size_t len, struct mg_dns_message *dm) {
  ...
  for (i = 0; i < mg_ntohs(h->num_answers); i++) {
    ...
    if (rr.alen == 4 && rr.atype == 1 && rr.aclass == 1) {
      ...
      os_memcpy(&dm->addr.ip, &buf[ofs - 4], 4);
      dm->resolved = true;
      break;  // Return success
    } else if (rr.alen == 16 && rr.atype == 28 && rr.aclass == 1) {
      dm->addr.is_ip6 = true;
      os_memcpy(&dm->addr.ip6, &buf[ofs - 16], 16);
      dm->resolved = true;
      break;  // Return success
    }
  }
...
void mg_dns_send(struct mg_connection *c, const struct mg_str *name,
  ...
  for (i = n = 0; i < sizeof(pkt.data) - 5; i++) {
    if (name->ptr[i] == '.' || i >= name->len) {
      ...
      os_memcpy(&pkt.data[n + 1], name->ptr + n, i - n);
      n = i + 1;
    }
    if (i >= name->len) break;
  }
  os_memcpy(&pkt.data[n], "\x00\x00\x01\x00\x01", 5);  // A query
  n += 5;
  if (ipv6) pkt.data[n - 3] = 0x1c;  // AAAA query
  // os_memcpy(&pkt.data[n], "\xc0\x0c\x00\x1c\x00\x01", 6);  // AAAA query
  // n += 6;
...
int mg_gethostbyname(const char *host,int port, struct sockaddr_in *sa){
	...
	if(port == -1){
        ...
        for (rp = res; rp != NULL; rp = rp->ai_next) {
		 ...
		 LOGW("mg_gethostbyname: %s ip: %s:%d", host,inet_ntoa(s4->sin_addr),htons(s4->sin_port));
		 n = 0;
                 break;
            }
        }
...
void mg_resolve(struct mg_connection *c, struct mg_str *name, int ms) {
  LOGW("%s %d  host name = %.*s\n", __func__, __LINE__,(int) name->len, name->ptr);
  if (mg_aton(*name, &c->peer)) {
    ...
    int result;
    struct mg_str ipname;
    char szservername[128] = {0};
    snprintf(szservername,128,"%.*s",(int) name->len, name->ptr);
    char szipaddr[64] = {0};
    struct sockaddr_in addrin;
    LOGW("mg_resolve name %s\n",szservername);
    result = mg_gethostbyname(szservername,-1,&addrin);
    if(result ==0){
	snprintf(szipaddr,64,"%s",inet_ntoa(addrin.sin_addr));
	ipname = mg_str(szipaddr);
	LOGW("mg_resolve ipaddr %s\n",szipaddr);
	if (mg_aton(ipname, &c->peer)) {
		 mg_connect_resolved(c);
	}else{
...
static int posix_stat(const char *path, size_t *size, time_t *mtime) {
  struct stat st;
  if (bk_vfs_stat(path, &st) != 0) return 0;
...
static void posix_list(const char *path, void (*fn)(const char *, void *),
  ...
  struct dirent *dp;
  DIR *dirp;
  if ((dirp = (bk_vfs_opendir(path))) == NULL) return;
  while ((dp = bk_vfs_readdir(dirp)) != NULL) {
    if (!strcmp(dp->d_name, ".") || !strcmp(dp->d_name, "..")) continue;
    fn(dp->d_name, userdata);
  }
  bk_vfs_closedir(dirp);
}
...
static struct mg_fd *posix_open(const char *path, int flags) {
  ...
  int mode = 0;
   if(flags == MG_FS_READ){
    ...
    }else if(flags == MG_FS_WRITE){
	mode = O_RDWR | O_CREAT | O_APPEND;
    }
  int fd_open = -1;
  struct mg_fd *fd = NULL;

  fd_open = bk_vfs_open(path, mode);

  if (fd_open <0 ) return NULL;
  fd = (struct mg_fd *) mg_calloc(1, sizeof(struct mg_fd));
  fd->fd = fd_open;
  fd->fs = &mg_fs_posix;
  return fd;
...
static size_t posix_read(int fd, void *buf, size_t len) {
   size_t readlen = bk_vfs_read(fd, buf, len);
   return readlen;
}

static size_t posix_write(int fd, const void *buf, size_t len) {

  return bk_vfs_write(fd, buf, len);
}

static size_t posix_seek(int fd, size_t offset) {
  off_t res =  bk_vfs_lseek( fd, offset, SEEK_SET);
...
void mg_http_bauth(struct mg_connection *c, const char *user,
  ...
  if (c->send.size >= need) {
    ...
    os_memcpy(&buf[-21], "Authorization: Basic ", 21);  // DON'T use mg_send!
    ...
    os_memcpy(&c->send.buf[c->send.len - 2], "\r\n", 2);
  } else {
...
int mg_http_upload(struct mg_connection *c, struct mg_http_message *hm,
  ...
  if (name[0] == '\0') {
    ...
    fd = bk_vfs_open(path, O_RDWR | O_CREAT | O_APPEND);
    if (fd<0) {
      mg_http_reply(c, 400, "", "fopen(%s): %d", name, errno);
      return -2;
    } else {
      bk_vfs_write(fd, hm->body.ptr, hm->body.len);
      bk_vfs_close(fd);
      mg_http_reply(c, 200, "", "");
...
static void static_cb(struct mg_connection *c, int ev, void *ev_data,
  ...
  if (ev == MG_EV_WRITE || ev == MG_EV_POLL) {
    ...
    n = fd->fs->mg_read(fd->fd, c->send.buf + c->send.len,
...
void mg_http_serve_file(struct mg_connection *c, struct mg_http_message *hm,
  ...
  struct mg_fd *fd = fs->mg_open(path, MG_FS_READ);
  size_t size = 0;
  time_t mtime = 0;
  struct mg_str *inm = NULL;
  if (fd == NULL || fs->mg_stat(path, &size, &mtime) == 0) {
    LOGW("%s %d  404 [%.*s]\n", __func__, __LINE__,(int) hm->uri.len, hm->uri.ptr);
...
static void printdirentry(const char *name, void *userdata) {
  ...
  } else if ((flags = fs->mg_stat(path, &size, &mtime)) == 0) {
...
static void listdir(struct mg_connection *c, struct mg_http_message *hm,
  ...
  fs->mg_list(dir, printdirentry, &d);
  mg_printf(c,
  ...
  if (n > sizeof(tmp)) n = 0;
  os_memcpy(c->send.buf + off - 10, tmp, n);  // Set content length
}
...
static int uri_to_path(struct mg_connection *c, struct mg_http_message *hm,
  ...
  struct mg_fs *fs = opts->fs == NULL ? &mg_fs_posix : opts->fs;
  int flags = 0, tmp;
  if (fs->mg_realpath(opts->root_dir, root_dir) == NULL) {
    LOG(LL_ERROR, ("realpath(%s): %d", opts->root_dir, errno));
    mg_http_reply(c, 400, "", "Bad web root [%s]\n", opts->root_dir);
  } else if (!(fs->mg_stat(root_dir, NULL, NULL) & MG_FS_DIR)) {
    ...
    // Try to resolve it...
    if (fs->mg_realpath(root_dir, path) == NULL ||
        (flags = fs->mg_stat(path, NULL, NULL)) == 0) {
	LOGW("%s %d  %s Not found \n", __func__, __LINE__,path);
      ...
      if ((flags & MG_FS_DIR) &&
          ((snprintf(path + n2, plen - n2, "/index.html") > 0 &&
            (tmp = fs->mg_stat(path, NULL, NULL)) != 0) ||
...
void mg_http_serve_dir(struct mg_connection *c, struct mg_http_message *hm,
  ...
  if (flags == 0) {
	LOGW("%s %d  root [%s], path [%s] %d\n", __func__, __LINE__,root, path, flags);
	return;
   }
  ...
  if (ev == MG_EV_READ || ev == MG_EV_CLOSE) {
    ...
    for (;;) {
      ...
      if (n < 0 && ev == MG_EV_READ) {
        //mg_error(c, "http_cb HTTP parse:\n%.*s", (int) c->recv.len, c->recv.buf);
	mg_error(c, "http_cb HTTP parse MG_EV_READ");
        break;
...
// Not using os_memset for zeroing memory, cause it can be dropped by compiler
...
int mg_iobuf_resize(struct mg_iobuf *io, size_t new_size) {
  ...
  if (new_size == 0) {
    ...
    if (p != NULL) {
      ...
      if (len > 0) os_memcpy(p, io->buf, len);
      zeromem(io->buf, io->size);
      free(io->buf);
...
void mg_md5_final(mg_md5_ctx *ctx, unsigned char digest[16]) {
  ...
  *p++ = 0x80;
  ...
  os_memset((char *) ctx, 0, sizeof(*ctx));
}
#endif
...
struct mg_connection *mg_mqtt_connect(struct mg_mgr *mgr, const char *url,
  ...
  if (c != NULL) {
    ...
    os_memset(&empty, 0, sizeof(empty));
...
static bool mg_aton4(struct mg_str str, struct mg_addr *addr) {
  ...
  os_memcpy(&addr->ip, data, sizeof(data));
  addr->is_ip6 = false;
  return true;
}
...
void mg_sha1_final(unsigned char digest[20], mg_sha1_ctx *context) {
  ...
  os_memset(&finalcount, '\0', sizeof(finalcount));
}
...
void mg_hmac_sha1(const unsigned char *key, size_t keylen,
  ...
  os_memset(buf1, 0, sizeof(buf1));
  os_memset(buf2, 0, sizeof(buf2));
  os_memcpy(buf1, key, keylen);
  os_memcpy(buf2, key, keylen);
...
static socklen_t tousa(struct mg_addr *a, union usa *usa) {
  ...
  *(uint32_t *) &usa->sin.sin_addr = a->ip;
  ...
  if (a->is_ip6) {
    ...
    os_memcpy(&usa->sin6.sin6_addr, a->ip6, sizeof(a->ip6));
    len = sizeof(usa->sin6);
  }
#endif
  return len;
...
static void tomgaddr(union usa *usa, struct mg_addr *a, bool is_ip6) {
  ...
  os_memcpy(&a->ip, &usa->sin.sin_addr, sizeof(a->ip));
#if MG_ENABLE_IPV6
  if (is_ip6) {
    os_memcpy(a->ip6, &usa->sin6.sin6_addr, sizeof(a->ip6));
...
#if ESP32 || ESP8266 || defined(__linux__) || BK_7258
  int idle = 60;
...
static bool mg_socketpair(SOCKET sp[2], union usa usa[2]) {
  ...
  (void) os_memset(&usa[0], 0, sizeof(usa[0]));
...
struct mg_connection *mg_listen(struct mg_mgr *mgr, const char *url,
  ...
  if (fd == INVALID_SOCKET) {
    ...
    os_memcpy(&c->peer, &addr, sizeof(struct mg_addr));
    c->fd = S2PTR(fd);
    c->is_listening = 1;
    c->is_udp = is_udp;
    LIST_ADD_HEAD(struct mg_connection, &mgr->conns, c);
...
static int net_would_block( void *ctx )
{
    int err = errno;
#if 0
    /*
...
int webrtc_mbedtls_net_recv( void *ctx, unsigned char *buf, size_t len )
{
   //LOGW("%s %d  \n", __func__, __LINE__);
...
int webrtc_mbedtls_net_send( void  *ctx, const unsigned char *buf, size_t len )
{
    //LOGW("%s %d  \n", __func__, __LINE__);
     struct mg_connection * c = (struct mg_connection *)ctx;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
void mg_tls_handshake(struct mg_connection *c) {
  ...
  mbedtls_ssl_set_bio(&tls->ssl, c, webrtc_mbedtls_net_send, webrtc_mbedtls_net_recv, 0);
...
static void debug_cb(void *c, int lev, const char *s, int n, const char *s2) {
  ...
  //LOG(LL_VERBOSE_DEBUG, ("%p %.*s", ((struct mg_connection *) c)->fd, n, s2));
   //LOGW("%p %.*s", ((struct mg_connection *) c)->fd, n, s2);
  (void) s;
  (void) c;
  (void) lev;
}
...
void mg_tls_init(struct mg_connection *c, struct mg_tls_opts *opts) {
  ...
  if (opts->ca != NULL && opts->ca[0] != '\0') {
    ...
    if (rc != 0) {
      mg_error(c, "parse on-disk chain(--) err %#x", -rc);
      goto fail;
    }
#else
...
#else
    rc =mbedtls_x509_crt_parse(&tls->ca, (uint8_t *) opts->ca,
                                      strlen(opts->ca) + 1);
#endif
    if (rc != 0) {
     // mg_error(c, "parse(%s) err %#x", ca, -rc);
	mg_error(c, "parse(--) err %#x", -rc);
      goto fail;
    }
...
#else
    if(opts->cert[0] == '-'){
    rc = mbedtls_x509_crt_parse(&tls->cert, (uint8_t *) opts->cert,
                                      strlen(opts->cert) + 1);
    }else{
...
#else
    rc = mbedtls_pk_parse_key(&tls->pk, (uint8_t *) key,
                                              strlen(key) + 1, NULL, 0 RNG);
#endif
    if (rc != 0) {
      mg_error(c, "tls key(--) %#x", -rc);
      goto fail;
    }
...
char *mg_file_read(const char *path, size_t *sizep) {
  ...
  if (fd>=0) {
    bk_vfs_lseek(fd, 0, SEEK_END);
    size = (size_t) bk_vfs_tell(fd);
    //bk_vfs_rewind(fd);
    bk_vfs_lseek(fd, 0, SEEK_SET);
    data = (char *) calloc(1, size + 1);
    if (data != NULL) {
      if (bk_vfs_read(fd,data, size) != size) {
        free(data);
        data = NULL;
      } else {
        data[size] = '\0';
...
bool mg_file_write(const char *path, const void *buf, size_t len) {
  ...
  fd = bk_vfs_open(path, O_RDWR | O_CREAT | O_APPEND);
  if (fd >=0) {
    result = bk_vfs_write(fd,buf,len) == len;
    bk_vfs_close(fd);
    if (result) {
      bk_vfs_unlink(path);
      bk_vfs_rename(tmp, path);
    } else {
      bk_vfs_unlink(tmp);
    }
  }
...
void mg_random(void *buf, size_t len) {
  ...
  while (len--) *p++ = (unsigned char) (esp_random() & 255);
	...
	while (len--) *p++ = (unsigned char) (bk_rand() & 255);
	done = true;          
#else
...
uint32_t mg_ntohl(uint32_t net) {
  uint8_t data[4] = {0, 0, 0, 0};
  os_memcpy(&data, &net, sizeof(data));
...
void mg_usleep(unsigned long usecs) {
  ...
  ets_delay_us(usecs);
#elif MG_ARCH == MG_ARCH_BK
  rtos_delay_milliseconds(usecs/1000);
...
unsigned long mg_millis(void) {
  ...
  return xTaskGetTickCount() * portTICK_PERIOD_MS;
#elif MG_ARCH == MG_ARCH_BK
   return (unsigned long)rtos_get_time();
#else
  struct timespec ts;
...
static size_t mkhdr(size_t len, int op, bool is_client, uint8_t *buf) {
  ...
  if (len < 126) {
    ...
    buf[1] = 126;
    os_memcpy(&buf[2], &tmp, sizeof(tmp));
    n = 4;
  } else {
    uint32_t tmp;
    buf[1] = 127;
    ...
    os_memcpy(&buf[6], &tmp, sizeof(tmp));
    n = 10;
  }
  if (is_client) {
...
size_t mg_ws_wrap(struct mg_connection *c, size_t len, int op) {
  ...
  os_memcpy(p - header_len, header, header_len);               // Prepend header
