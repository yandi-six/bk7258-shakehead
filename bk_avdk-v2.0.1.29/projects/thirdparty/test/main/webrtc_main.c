...
static void webrtc_wifi_event_cb(void *new_evt)
{
	...
	{
		wifi_connected = 0;
		webrtc_mailbox_send_netfail_msg(); //
		LOGW("密码错误\r\n");
	}
	break;
...
void webrtc_media_yuv_thread(void *param)
{
	...
	{
		// LOGW("frame_buffer_fb_display_pop_wait\n");
		...
		if(1 < webrtc_streamer_current_session_count())
		{
			...
			// rtc_list_for_each4(livelist,webrtc_streamer_close_session,LiveSessionId);
			// rtc_list_free(livelist);
			// livelist = NULL;
			// rtc_list_append(livelist,LiveSessionId);
				...
				{ 	
					...
					if (webrtc_recording && pdata->isvideo && pdata->data != NULL && (0x07 == (pdata->data[4] & 0x1f)))
					{
						...
						else if(time_str == 'b')
						{
							...
							if(Ttime - Ftime > 3000){
								i_count_delay = 2596;//depend on p count
							}else
...
void webrtc_network_configure_update()
{
	...
	*/
	...
	if (pwbuf != NULL)
	{
		...
		snprintf(szurl, 256, "%s/v1/device/confirm_bind",config_api_url);//config_api_url api.newaylink.com
		bk_printf("szurl:%s\n",szurl);
		bk_printf("pdata:%s\n",pdata);
		os_free(pwbuf);
		pwbuf = NULL;
...
void runhua_main(void)
{
	...
	while (runhua_runing)
	{
		...
		if (webrtc_audio_playing_file == false && play_file_index == -1)
		{
			...
			if ( webrtc_streamer_media_start == false)//webrtc_streamer_online == true &&
			{
...
// CPU0卸载SD卡 发送消息给CPU1令其停止 关闭cpu1 恢复cpu1邮件通信到初始状态 停止MDNS服务
	...
	if (webrtc_record_sem != NULL)
	{
		...
		{
			bk_printf("get_semaphore webrtc_record_exit_sem\n");
			rtos_get_semaphore(&webrtc_record_exit_sem, BEKEN_WAIT_FOREVER);
			bk_printf("deinit_semaphore webrtc_record_exit_sem\n");
	...
	if (webrtc_audio_play_sem != NULL)
	{
		...
		if (webrtc_audio_play_exit_sem != NULL)
		{
			bk_printf("get_semaphore webrtc_audio_play_exit_sem\n");
			rtos_get_semaphore(&webrtc_audio_play_exit_sem, BEKEN_WAIT_FOREVER);
			bk_printf("deinit_semaphore webrtc_audio_play_exit_sem\n");
