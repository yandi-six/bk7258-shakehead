...
static inline void *mg_calloc(size_t n, size_t size) {
...
static inline void *mg_malloc(size_t size)
{
#if CONFIG_PSRAM_AS_SYS_MEMORY
...
#define free(a) mg_free(a)
#define malloc(a) mg_malloc(a)

#define gmtime_r(a, b) gmtime(a)

#endif  // MG_ARCH == MG_ARCH_BK
...
#if MG_ARCH == MG_ARCH_BK
struct mg_fs {
  char *(*mg_realpath)(const char *path, char *resolved_path);
  int (*mg_stat)(const char *path, size_t *size, time_t *mtime);
  void (*mg_list)(const char *path, void (*fn)(const char *, void *), void *);
  struct mg_fd *(*mg_open)(const char *path, int flags);
  void (*mg_close)(struct mg_fd *fd);
  size_t (*mg_read)(int fd, void *buf, size_t len);
  size_t (*mg_write)(int fd, const void *buf, size_t len);
  size_t (*mg_seek)(int fd, size_t offset);
};
#else
struct mg_fs {
  char *(*mg_realpath)(const char *path, char *resolved_path);
  ...
  struct mg_fd *(*mg_open)(const char *path, int flags);
  void (*mg_close)(struct mg_fd *fd);
  size_t (*mg_read)(void *fd, void *buf, size_t len);
  size_t (*mg_write)(void *fd, const void *buf, size_t len);
  size_t (*mg_seek)(void *fd, size_t offset);
};
#endif
// File descriptor
struct mg_fd {
