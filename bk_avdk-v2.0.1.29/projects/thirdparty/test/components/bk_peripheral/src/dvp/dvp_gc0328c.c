...
#if 1
const UINT8 sensor_gc0328c_init_talbe[][2] = {
	...
	{0x70, 0x40}, //45  add for xia 45->25
	...
	{0x32, 0x40}, //
	{0x33, 0x30}, //0x20选择曝光等级 add for xia  20->00
	...
	//////////// block enable/////////////
	{0x40, 0x7f}, // 0x7f    //第二位代表去噪模块的使能add for xia
	{0x41, 0x26},
	{0x42, 0xfb},
	{0x44, 0x02}, // uyvy
	{0x45, 0x00},
	...
	{0x09, 0xe8},
	{0x10, 0x08},
	{0x11, 0x21},  //AEC每隔几帧调一次add xia
	{0x12, 0x11},
	{0x13, 0x38}, // Y target 45
	{0x15, 0xfc},
	{0x18, 0x02},
	{0x21, 0xf0},
	{0x22, 0x60},
...
const uint8_t sensor_gc0328c_VGA_640_368_talbe[][2] = //+++
	{
#if (GC_QVGA_USE_SUBSAMPLE == 1)
		{0xFE, 0x00},
		{0x59, 0x11},
...
int gc0328c_set_ppi(media_ppi_t ppi)
{
	...
	case PPI_640X368:
	{
		size = sizeof(sensor_gc0328c_VGA_640_368_talbe) / 2;

		for (i = 0; i < size; i++)
		{
			SENSOR_I2C_WRITE(sensor_gc0328c_VGA_640_368_talbe[i][0],
							 sensor_gc0328c_VGA_640_368_talbe[i][1]);

			gc0328c_read_register(sensor_gc0328c_VGA_640_368_talbe[i][0],
								  sensor_gc0328c_VGA_640_368_talbe[i][1]);
		}
...
int gc0328c_set_flip(int mirror, int vertical)
{
	...
	{
		SENSOR_I2C_WRITE(0x17, GC0328_FLIP_INIT);
	}
	else if (mirror == 1 && vertical == 0)
	{

		SENSOR_I2C_WRITE(0x17, GC0328_FLIP_INIT ^ 0x01);
	}
	else if (mirror == 0 && vertical == 1)
	{

		SENSOR_I2C_WRITE(0x17, GC0328_FLIP_INIT ^ 0x02);
	}
	else if (mirror == 1 && vertical == 1)
	{

		SENSOR_I2C_WRITE(0x17, GC0328_FLIP_INIT ^ 0x03);
	}
	else
	{
		LOGI("flip set err\n");
...
const dvp_sensor_config_t dvp_sensor_gc0328c =
	{
		...
		.ppi_cap = PPI_CAP_320X240 | PPI_CAP_640X368 | PPI_CAP_320X480 | PPI_CAP_480X272 | PPI_CAP_640X480 | PPI_CAP_480X320,
		.id = ID_GC0328C,
