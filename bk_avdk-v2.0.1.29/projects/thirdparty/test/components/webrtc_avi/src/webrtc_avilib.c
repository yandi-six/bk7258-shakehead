...
static size_t webrtc_avi_read(int fd, char *buf, size_t len)
{
   size_t n = 0;
   size_t r = 0;

   while (r < len) {
      n = vfs_file_read (fd, buf + r, len - r);
...
static size_t webrtc_avi_write (int fd, char *buf, size_t len)
{
   size_t n = 0;
   size_t r = 0;

   while (r < len) {
      n = vfs_file_write (fd, buf + r, len - r);
      if ((ssize_t)n < 0)
         return n;
...
static int webrtc_avi_sampsize(webrtc_avi_t *AVI, int j)
{
   int s;
...
static int webrtc_avi_add_chunk(webrtc_avi_t *AVI, unsigned char *tag, unsigned char *data, int length)
{
   unsigned char c[8];
   ...
   if( webrtc_avi_write(AVI->fdes,(char *)c,8) != 8 ||
       webrtc_avi_write(AVI->fdes,(char *)data,length) != length )
   {
      vfs_file_lseek(AVI->fdes,AVI->pos,SEEK_SET);
...
static int webrtc_avi_add_index_entry(webrtc_avi_t *AVI, unsigned char *tag, long flags, unsigned long pos, unsigned long len)
{
   void *ptr;

   if(AVI->n_idx>=AVI->max_idx) {
     ptr = rtc_bk_realloc((void *)AVI->idx,(AVI->max_idx+4096)*16);
   ...
   rtc_bk_memcpy(AVI->idx[AVI->n_idx],tag,4);
...
/*
   ...
   returns a pointer to webrtc_avi_t on success, a zero pointer on error
*/

webrtc_avi_t* WEBRTC_AVI_open_output_file(char * filename)
{
   webrtc_avi_t *AVI;
   int i;
   unsigned char *AVI_header = (unsigned char *)rtc_bk_malloc(HEADERBYTES);
   if(AVI_header == NULL){
      AVI_errno = AVI_ERR_NO_MEM;
      return NULL;
   }

   /* Allocate the webrtc_avi_t struct and zero it */

   AVI = (webrtc_avi_t *) rtc_bk_malloc(sizeof(webrtc_avi_t));
   if(AVI==NULL)
   {
      if(AVI_header!= NULL){
		rtc_bk_free(AVI_header);
		AVI_header = NULL;
      }
      AVI_errno = AVI_ERR_NO_MEM;
      return NULL;
   }
   rtc_bk_memset((void *)AVI,0,sizeof(webrtc_avi_t));
   ...
   AVI->fdes = vfs_file_open(filename, O_RDWR | O_CREAT);
   if (AVI->fdes < 0)
   {
      if(AVI_header!= NULL){
		rtc_bk_free(AVI_header);
   ...
   rtc_bk_memset((void *)AVI_header,0,HEADERBYTES);
   
   i = webrtc_avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES);
   if (i != HEADERBYTES)
   {
      if(AVI_header!= NULL){
		rtc_bk_free(AVI_header);
...
void WEBRTC_AVI_set_video(webrtc_avi_t *AVI, int width, int height, double fps, char *compressor)
{
   ...
   if(strncmp(compressor, "RGB", 3)==0) {
     rtc_bk_memset(AVI->compressor, 0, 4);
   } else {
     rtc_bk_memcpy(AVI->compressor,compressor,4);
   }     
   
   AVI->compressor[4] = 0;

   webrtc_avi_update_header(AVI);
}

void WEBRTC_AVI_set_audio(webrtc_avi_t *AVI, int channels, long rate, int bits, int format, long mp3rate)
{
   /* may only be called if file is open for writing */
   ...
   if(AVI->anum > AVI_MAX_TRACKS) {
     LOGE("error - only %d audio tracks supported\n", AVI_MAX_TRACKS);
     return;
   }
...
#define OUT4CC(s) \
   if(nhb<=HEADERBYTES-4) rtc_bk_memcpy(AVI_header+nhb,s,4); nhb += 4
...
int webrtc_avi_update_header(webrtc_avi_t *AVI)
{
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   for(j=0; j<AVI->anum; ++j) {
       ...
       sampsize = webrtc_avi_sampsize(AVI, j);
   ...
   if(njunk<=0)
     {
       LOGE("AVI_close_output_file: # of header bytes too small\n");
       return 0;
     }
   
   OUT4CC ("JUNK");
   OUTLONG(njunk);
   rtc_bk_memset(AVI_header+nhb,0,njunk);
   
   //2001-11-14 added id string 

   if(njunk > strlen(id_str)+8) {
     //sprintf(id_str, "%s-%s", PACKAGE, VERSION);
     //rtc_bk_memcpy(AVI_header+nhb, id_str, strlen(id_str));
   }
   ...
   if ( vfs_file_lseek(AVI->fdes,0,SEEK_SET)<0 ||
        webrtc_avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES)!=HEADERBYTES ||
	vfs_file_lseek(AVI->fdes,AVI->pos,SEEK_SET)<0)
     {
      if(AVI_header!= NULL){
		rtc_bk_free(AVI_header);
		AVI_header = NULL;
...
static int webrtc_avi_close_output_file(webrtc_avi_t *AVI)
{
...
//   time_t calptr;
   ...
   ret = webrtc_avi_add_chunk(AVI, (unsigned char *)"idx1", (unsigned char *)AVI->idx, AVI->n_idx*16);
   hasIndex = (ret==0);
...
//   sprintf(id_str, "\t");
//   rtc_bk_memset(AVI_header+nhb, 0, MAX_INFO_STRLEN);
//   rtc_bk_memcpy(AVI_header+nhb, id_str, strlen(id_str));
//   nhb += MAX_INFO_STRLEN;
...
//   rtc_bk_memset(AVI_header+nhb, 0, MAX_INFO_STRLEN);
//   rtc_bk_memcpy(AVI_header+nhb, id_str, 25);
//   nhb += MAX_INFO_STRLEN;
#endif
...
static int webrtc_avi_write_data(webrtc_avi_t *AVI, char *data, unsigned long length, int audio, int keyframe)
{
   int n;
   ...
   if(audio)
     n = webrtc_avi_add_index_entry(AVI,astr,0x00,AVI->pos,length);
   else
     n = webrtc_avi_add_index_entry(AVI,(unsigned char *)"00db",((keyframe)?0x10:0x0),AVI->pos,length);
   ...
   if(audio)
     n = webrtc_avi_add_chunk(AVI,astr,(unsigned char *)data,length);
   else
     n = webrtc_avi_add_chunk(AVI,(unsigned char *)"00db",(unsigned char *)data,length);
...
int WEBRTC_AVI_write_frame(webrtc_avi_t *AVI, char *data, long bytes, int keyframe)
{
  unsigned long pos;
  ...
  if(webrtc_avi_write_data(AVI,data,bytes,0,keyframe)) return -1;
...
int WEBRTC_AVI_dup_frame(webrtc_avi_t *AVI)
{
   if(AVI->mode==AVI_MODE_READ) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }

   if(AVI->last_pos==0) return 0; /* No previous real frame */
   if(webrtc_avi_add_index_entry(AVI,(unsigned char *)"00db",0x10,AVI->last_pos,AVI->last_len)) return -1;
   AVI->video_frames++;
   AVI->must_use_index = 1;
   return 0;
}

int WEBRTC_AVI_write_audio(webrtc_avi_t *AVI, char *data, long bytes)
{
   if(AVI->mode==AVI_MODE_READ) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }

   if( webrtc_avi_write_data(AVI,data,bytes,1,0) ) return -1;
   AVI->track[AVI->aptr].audio_bytes += bytes;
   return 0;
}


int WEBRTC_AVI_append_audio(webrtc_avi_t *AVI, char *data, long bytes)
{
  ...
  //update chunk header
  vfs_file_lseek(AVI->fdes, pos+4, SEEK_SET);
  long2str(c, length+bytes);     
  webrtc_avi_write(AVI->fdes, (char *)c, 4);

  vfs_file_lseek(AVI->fdes, pos+8+length, SEEK_SET);

  i=PAD_EVEN(length + bytes);

  bytes = i - length;
  webrtc_avi_write(AVI->fdes, data, bytes);
  AVI->pos = pos + 8 + i;
...
long WEBRTC_AVI_bytes_remain(webrtc_avi_t *AVI)
{
...
long WEBRTC_AVI_bytes_written(webrtc_avi_t *AVI)
{
   if(AVI->mode==AVI_MODE_READ) return 0;
...
int WEBRTC_AVI_set_audio_track(webrtc_avi_t *AVI, int track)
{
...
int WEBRTC_AVI_get_audio_track(webrtc_avi_t *AVI)
{
    return(AVI->aptr);
}
...
int WEBRTC_AVI_close(webrtc_avi_t *AVI)
{
   int ret, i;
   ...
   if(AVI->mode == AVI_MODE_WRITE)
      ret = webrtc_avi_close_output_file(AVI);
   else
      ret = 0;
   ...
   vfs_file_close(AVI->fdes);
   if(AVI->idx) rtc_bk_free(AVI->idx);
   if(AVI->video_index) rtc_bk_free(AVI->video_index);
   //FIXME
   //if(AVI->audio_index) rtc_bk_free(AVI->audio_index);
   if (AVI->bitmap_info_header)
     rtc_bk_free(AVI->bitmap_info_header);
   for (i = 0; i < AVI->anum; i++) {
     if (AVI->wave_format_ex[i])
       rtc_bk_free(AVI->wave_format_ex[i]);
     if (AVI->track[i].audio_chunks)
       rtc_bk_free(AVI->track[i].audio_index);
   }
   rtc_bk_free(AVI);
...
#define ERR_EXIT(x) \
{ \
   WEBRTC_AVI_close(AVI); \
   AVI_errno = x; \
   return 0; \
}

webrtc_avi_t *WEBRTC_AVI_open_input_file(const char *filename, int getIndex)
{
  int file_is_ok = 0;
  unsigned int size;
  webrtc_avi_t *AVI=NULL;
  
  /* Create webrtc_avi_t structure */
  ...
  AVI->fdes = vfs_file_open(filename,O_RDONLY);
  ...
  vfs_file_lseek(AVI->fdes, 0L, SEEK_END);
  size = vfs_file_tell(AVI->fdes);
  vfs_file_lseek(AVI->fdes, 0L, SEEK_SET);
  if(size<=0){
	LOGE("%s %d  failed size open %s is 0\n", __func__, __LINE__,filename);
	WEBRTC_AVI_close(AVI);
	AVI_errno = AVI_ERR_OPEN;
        return NULL;
  }

  file_is_ok =WEBRTC_avi_parse_input_file(AVI, getIndex);
  if(file_is_ok<=0){
	LOGE("%s %d  failed avi_parse_input_file %s\n", __func__, __LINE__,filename);
	WEBRTC_AVI_close(AVI);
	AVI_errno = AVI_ERR_OPEN;
        return NULL;
  }
...
webrtc_avi_t *WEBRTC_AVI_open_fd(int fd, int getIndex)
{
  webrtc_avi_t *AVI=NULL;
  int file_is_ok = 0;
  /* Create webrtc_avi_t structure */
...
int WEBRTC_avi_parse_input_file(webrtc_avi_t *AVI, int getIndex)
{
  //LOGW("%s %d  \n", __func__, __LINE__);
  ...
  /* Read first 12 bytes and check that this is an AVI file */
   memset(data,0,256);
   if( webrtc_avi_read(AVI->fdes,data,12) != 12 ) ERR_EXIT(AVI_ERR_READ)
   ...
   {
      memset(data,0,256);
      if( webrtc_avi_read(AVI->fdes,data,8) != 8 ) break; /* We assume it's EOF */
      ...
      {
	 memset(data,0,256);
         if( webrtc_avi_read(AVI->fdes,data,4) != 4 ) ERR_EXIT(AVI_ERR_READ)
         n -= 4;
         if(strncasecmp(data,"hdrl",4) == 0)
         {
            hdrl_len = n;
            hdrl_data = (unsigned char *) rtc_bk_malloc(n);
            if(hdrl_data==0) ERR_EXIT(AVI_ERR_NO_MEM);
				 
	    // offset of header
	    
	    header_offset = vfs_file_lseek(AVI->fdes,0,SEEK_CUR);	 
            if( webrtc_avi_read(AVI->fdes,(char *)hdrl_data,n) != n ) ERR_EXIT(AVI_ERR_READ)
         }
         else if(strncasecmp(data,"movi",4) == 0)
         {
            AVI->movi_start = vfs_file_lseek(AVI->fdes,0,SEEK_CUR);
            vfs_file_lseek(AVI->fdes,n,SEEK_CUR);
         }
         else
            vfs_file_lseek(AVI->fdes,n,SEEK_CUR);
      }
      else if(strncasecmp(data,"idx1",4) == 0)
      {
         ...
         AVI->idx = (unsigned  char((*)[16]) ) rtc_bk_malloc(n);
         if(AVI->idx==0) ERR_EXIT(AVI_ERR_NO_MEM)
         if(webrtc_avi_read(AVI->fdes, (char *) AVI->idx, n) != n ) ERR_EXIT(AVI_ERR_READ)
      }
      else
         vfs_file_lseek(AVI->fdes,n,SEEK_CUR);
   }
   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      if(strncasecmp((char *)hdrl_data+i,"strh",4)==0)
      {
         ...
         {
            rtc_bk_memcpy(AVI->compressor,hdrl_data+i+4,4);
            AVI->compressor[4] = 0;
	   ...
	   AVI->track[AVI->aptr].audio_bytes = str2ulong(hdrl_data+i+32)*webrtc_avi_sampsize(AVI, 0);
      ...
      else if(strncasecmp((char *)hdrl_data+i,"strf",4)==0)
      {
         ...
         if(lasttag == 1)
         {
            WEBRTC_BITMAPINFOHEADER_avilib bih;
            
            rtc_bk_memcpy(&bih, hdrl_data + i, sizeof(WEBRTC_BITMAPINFOHEADER_avilib));
            AVI->bitmap_info_header = (WEBRTC_BITMAPINFOHEADER_avilib *)rtc_bk_malloc(bih.bi_size);
            if (AVI->bitmap_info_header != NULL)
              rtc_bk_memcpy(AVI->bitmap_info_header, hdrl_data + i, bih.bi_size);
	    ...
	    rtc_bk_memcpy(AVI->compressor2, hdrl_data+i+16, 4);
            AVI->compressor2[4] = 0;

         }
         else if(lasttag == 2)
         {
            WEBRTC_WAVEFORMATEX_avilib *wfe;
	    char *nwfe;
            int wfes;
            
            if ((hdrl_len - i) < sizeof(WEBRTC_WAVEFORMATEX_avilib))
              wfes = hdrl_len - i;
            else
              wfes = sizeof(WEBRTC_WAVEFORMATEX_avilib);
            wfe = (WEBRTC_WAVEFORMATEX_avilib *)rtc_bk_malloc(sizeof(WEBRTC_WAVEFORMATEX_avilib));
            if (wfe != NULL) {
              rtc_bk_memset(wfe, 0, sizeof(WEBRTC_WAVEFORMATEX_avilib));
	      rtc_bk_memcpy(wfe, hdrl_data + i, wfes);
	      if (wfe->cb_size != 0) {
		nwfe = (char *)rtc_bk_realloc(wfe, sizeof(WEBRTC_WAVEFORMATEX_avilib) +
				       wfe->cb_size);
		if (nwfe != 0) {
		  off_t lpos = vfs_file_lseek(AVI->fdes, 0, SEEK_CUR);
		  vfs_file_lseek(AVI->fdes, header_offset + i + sizeof(WEBRTC_WAVEFORMATEX_avilib),
			SEEK_SET);
		  wfe = (WEBRTC_WAVEFORMATEX_avilib *)nwfe;
		  nwfe = &nwfe[sizeof(WEBRTC_WAVEFORMATEX_avilib)];
		  webrtc_avi_read(AVI->fdes, nwfe, wfe->cb_size);
		  vfs_file_lseek(AVI->fdes, lpos, SEEK_SET);
		}
	      }
   ...
   vfs_file_lseek(AVI->fdes,AVI->movi_start,SEEK_SET);
  ...
  //LOGW("%s %d  \n", __func__, __LINE__);
   ...
   if(AVI->idx)
   {
      ...
      vfs_file_lseek(AVI->fdes,pos,SEEK_SET);
      memset(data,0,256);
      if(webrtc_avi_read(AVI->fdes,data,8)!=8) ERR_EXIT(AVI_ERR_READ)
      ...
      {
         vfs_file_lseek(AVI->fdes,pos+AVI->movi_start-4,SEEK_SET);
	 memset(data,0,256);
         if(webrtc_avi_read(AVI->fdes,data,8)!=8) ERR_EXIT(AVI_ERR_READ)
         if( strncasecmp(data,(char *)AVI->idx[i],4)==0 && str2ulong((unsigned char *)data+4)==len )
         {
   ...
   if(idx_type == 0)
   {
      ...
      {
	 memset(data,0,256);
         if( webrtc_avi_read(AVI->fdes,data,8) != 8 ) break;
         n = str2ulong((unsigned char *)data+4);
         ...
         if(strncasecmp(data,"LIST",4)==0)
         {
            vfs_file_lseek(AVI->fdes,4,SEEK_CUR);
            continue;
         }
	     ...
	     || ( (data[2]=='w' || data[2]=='W') &&
		  (data[3]=='b' || data[3]=='B') ) )
	   {
	   webrtc_avi_add_index_entry(AVI,(unsigned char *)data,0,vfs_file_lseek(AVI->fdes,0,SEEK_CUR)-8,n);
         }
	 
         vfs_file_lseek(AVI->fdes,PAD_EVEN(n),SEEK_CUR);
      }
      idx_type = 1;
   }
...
//   fprintf(stderr, "chunks = %ld %d %s\n", AVI->track[0].audio_chunks, AVI->anum, AVI->track[0].audio_tag);
   ...
   AVI->video_index = (webrtc_video_index_entry *) rtc_bk_malloc(nvi*sizeof(webrtc_video_index_entry));
   if(AVI->video_index==0) ERR_EXIT(AVI_ERR_NO_MEM);
   
   for(j=0; j<AVI->anum; ++j) {
       if(AVI->track[j].audio_chunks) {
	   AVI->track[j].audio_index = (webrtc_audio_index_entry *) rtc_bk_malloc((nai[j]+1)*sizeof(webrtc_audio_index_entry));
	   rtc_bk_memset(AVI->track[j].audio_index, 0, (nai[j]+1)*(sizeof(webrtc_audio_index_entry)));
...
long WEBRTC_AVI_video_frames(webrtc_avi_t *AVI)
{
   return AVI->video_frames;
}
int  WEBRTC_AVI_video_width(webrtc_avi_t *AVI)
{
   return AVI->width;
}
int  WEBRTC_AVI_video_height(webrtc_avi_t *AVI)
{
   return AVI->height;
}
double WEBRTC_AVI_frame_rate(webrtc_avi_t *AVI)
{
   return AVI->fps;
}
char* WEBRTC_AVI_video_compressor(webrtc_avi_t *AVI)
{
   return AVI->compressor2;
}

long WEBRTC_AVI_max_video_chunk(webrtc_avi_t *AVI)
{
   return AVI->max_len; 
}

int WEBRTC_AVI_audio_tracks(webrtc_avi_t *AVI)
{
    return(AVI->anum);
}

int WEBRTC_AVI_audio_channels(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].a_chans;
}

long WEBRTC_AVI_audio_mp3rate(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].mp3rate;
}

int WEBRTC_AVI_audio_bits(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].a_bits;
}

int WEBRTC_AVI_audio_format(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].a_fmt;
}

long WEBRTC_AVI_audio_rate(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].a_rate;
}

long WEBRTC_AVI_audio_bytes(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].audio_bytes;
}

long WEBRTC_AVI_audio_chunks(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].audio_chunks;
}

long WEBRTC_AVI_audio_codech_offset(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].a_codech_off;
}

long WEBRTC_AVI_audio_codecf_offset(webrtc_avi_t *AVI)
{
   return AVI->track[AVI->aptr].a_codecf_off;
}

long  WEBRTC_AVI_video_codech_offset(webrtc_avi_t *AVI)
{
    return AVI->v_codech_off;
}

long  WEBRTC_AVI_video_codecf_offset(webrtc_avi_t *AVI)
{
    return AVI->v_codecf_off;
}

long WEBRTC_AVI_frame_size(webrtc_avi_t *AVI, long frame)
{
...
long WEBRTC_AVI_audio_size(webrtc_avi_t *AVI, long frame)
{
  if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
...
long WEBRTC_AVI_get_video_position(webrtc_avi_t *AVI, long frame)
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
   if(!AVI->video_index)         { AVI_errno = AVI_ERR_NO_IDX;   return -1; }
...
int WEBRTC_AVI_seek_start(webrtc_avi_t *AVI)
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
...
int WEBRTC_AVI_set_video_position(webrtc_avi_t *AVI, long frame)
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
   if(!AVI->video_index)         { AVI_errno = AVI_ERR_NO_IDX;   return -1; }
...
int WEBRTC_AVI_set_audio_bitrate(webrtc_avi_t *AVI, long bitrate)
{
   if(AVI->mode==AVI_MODE_READ) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
...
long WEBRTC_AVI_read_frame(webrtc_avi_t *AVI, char *vidbuf, int *keyframe)
{
   long n;
   ...
   *keyframe = (AVI->video_index[AVI->video_pos].key==0x10) ? 1:0;
   ...
   vfs_file_lseek(AVI->fdes, AVI->video_index[AVI->video_pos].pos, SEEK_SET);

   if (webrtc_avi_read(AVI->fdes,vidbuf,n) != n)
   {
...
int WEBRTC_AVI_set_audio_position(webrtc_avi_t *AVI, long byte)
{
   long n0, n1, n;
...
long WEBRTC_AVI_read_audio(webrtc_avi_t *AVI, char *audbuf, long bytes)
{
   ...
   while(bytes>0)
   {
      ...
      vfs_file_lseek(AVI->fdes, pos, SEEK_SET);
      if (webrtc_avi_read(AVI->fdes,audbuf+nr,todo) != todo)
      {
         AVI_errno = AVI_ERR_READ;
         return -1;
      }
...
long WEBRTC_AVI_read_audio_chunk(webrtc_avi_t *AVI, char *audbuf)
{
   long pos, left;
   ...
   pos = AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].pos + AVI->track[AVI->aptr].audio_posb;
   vfs_file_lseek(AVI->fdes, pos, SEEK_SET);
   if (webrtc_avi_read(AVI->fdes,audbuf,left) != left)
   {
      AVI_errno = AVI_ERR_READ;
      return -1;
   }
...
int WEBRTC_AVI_set_video_position_ex(webrtc_avi_t *AVI, long frame)
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
   if(!AVI->video_index)         { AVI_errno = AVI_ERR_NO_IDX;   return -1; }
   ...
   if (frame < 0 ) frame = 0;
   AVI->video_pos = frame;
   vfs_file_lseek(AVI->fdes, AVI->video_index[AVI->video_pos].pos-8, SEEK_SET);
   return 0;
}
...
int WEBRTC_AVI_read_data(webrtc_avi_t *AVI, char *vidbuf, long max_vidbuf,
 ...
 */
   ...
   while(1)
   {
      ...
      if( webrtc_avi_read(AVI->fdes,data,8) != 8 ) return 0;
      ...
      if(strncasecmp(data,AVI->video_tag,3) == 0)
      {
         ...
         *keyframe = (AVI->video_index[AVI->video_pos].key==0x10) ? 1:0;
         ...
         if(webrtc_avi_read(AVI->fdes,vidbuf,n) != n ) return 0;
         return 1;
      }
      else if(strncasecmp(data,AVI->track[AVI->aptr].audio_tag,4) == 0)
      {
         *len = n;
         ...
         if(webrtc_avi_read(AVI->fdes,audbuf,n) != n ) return 0;
         return 2;
         break;
      }
      else
         if(vfs_file_lseek(AVI->fdes,n,SEEK_CUR)<0)  return 0;
   }
}

uint64_t WEBRTC_AVI_max_size()
{
