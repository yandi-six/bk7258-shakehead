...
#define AUD_TRAS_DRV_TAG "aud_drv"
...
static void uac_mic_spk_count_close(void)
{
	...
	uac_mic_spk_count.mic_size = 0;
	uac_mic_spk_count.spk_size = 0;
	LOGI("close uac count timer complete \r\n");
}
...
static void uac_mic_spk_count_open(void)
{
	...
	if (uac_mic_spk_count.timer.handle != NULL)
	{
		...
		if (BK_OK != ret)
		{
			LOGE("deinit uac_mic_spk_count time fail\r\n");
			goto exit;
		}
		uac_mic_spk_count.timer.handle = NULL;
	}
	...
	LOGI("open uac count timer complete \r\n");
...
static bk_err_t aud_tras_aec(void)
{
	...
	rtc_bk_memcpy((void *)aud_tras_drv_info.voc_info.aec_dump.mic_dump_addr, aec_info_pr->mic_addr, aec_info_pr->samp_rate_points*2);
	rtc_bk_memcpy((void *)aud_tras_drv_info.voc_info.aec_dump.ref_dump_addr, aec_info_pr->ref_addr, aec_info_pr->samp_rate_points*2);
...
#if CONFIG_AUD_TRAS_AEC_DUMP_DEBUG
#if CONFIG_AUD_TRAS_AEC_DUMP_MODE_TF
	rtc_bk_memcpy((void *)aud_tras_drv_info.voc_info.aec_dump.out_dump_addr, aec_info_pr->out_addr, aec_info_pr->samp_rate_points*2);
#endif //CONFIG_AUD_TRAS_AEC_DUMP_MODE_TF
...
static void aud_uac_free_packet(audio_packet_t *packet)
{
	...
	if (packet->data_buffer_size == aud_tras_drv_info.spk_info.frame_size) {
		rtc_bk_memcpy(packet->data_buffer, aud_tras_drv_info.spk_info.uac_spk_buff, packet->data_buffer_size);
...
//	LOGI("%s size %x\r\n",__func__, packet->data_buffer_size);
	...
	if (aud_tras_drv_info.voc_info.status == AUD_TRAS_DRV_VOC_STA_START && aud_tras_drv_info.voc_info.speaker_samp_rate_points*2 == packet->data_buffer_size) {
		rtc_bk_memcpy(packet->data_buffer, aud_tras_drv_info.voc_info.uac_spk_buff, packet->data_buffer_size);
...
bk_err_t aud_tras_uac_auto_connect_ctrl(bool en)
{
...
static bk_err_t aud_tras_uac_connect_handle(void)
{
	...
	if (aud_tras_drv_info.uac_status == AUD_INTF_UAC_ABNORMAL_DISCONNECTED) {
		...
		if (!aud_tras_drv_info.uac_auto_connect) {
			LOGI("[%s] uac not automatically connect, need user todo \n", __func__);
			return BK_OK;
		}
	}
	...
	if (aud_tras_drv_info.work_mode == AUD_INTF_WORK_MODE_GENERAL) {
		...
		if (ret != BK_OK) {
			LOGE("uac set param fail \r\n");
			err = BK_ERR_AUD_INTF_UAC_DRV;
			goto fail;
		}
	}

	LOGI("voc_ops: %p, malloc: %p, push: %p, free: %p \r\n", &voc_uac_buffer_ops_funcs, voc_uac_buffer_ops_funcs._uac_malloc, voc_uac_buffer_ops_funcs._uac_push, voc_uac_buffer_ops_funcs._uac_free);
	...
	if (ret != BK_OK) {
		LOGE("register transfer buffer ops fail \r\n");
		err = BK_ERR_AUD_INTF_UAC_DRV;
		goto fail;
	}
	...
	if (aud_tras_drv_info.uac_mic_open_status == true && aud_tras_drv_info.uac_mic_open_current == false) {
		...
		if (ret != BK_OK) {
			LOGE("[%s] start uac mic fail, ret:%d \r\n", __func__, ret);
			err = BK_ERR_AUD_INTF_UAC_MIC;
			goto fail;
		} else {
	...
	if (aud_tras_drv_info.uac_spk_open_status == true && aud_tras_drv_info.uac_spk_open_current == false) {
		...
		if (ret != BK_OK) {
			LOGE("[%s] start uac spk fail, ret:%d \r\n", __func__, ret);
			err = BK_ERR_AUD_INTF_UAC_SPK;
			goto fail;
		} else {
...
//	uint32_t fill_size = 0;
...
#if AUD_MIC_DEBUG
			rtc_bk_memcpy(aud_tras_drv_info.voc_info.encoder_temp.pcm_data, PCM_8000, 320);
#endif
		}
	} else {
		if (aud_tras_drv_info.voc_info.aec_enable) {
	...
	switch (aud_tras_drv_info.voc_info.data_type) {
		case AUD_INTF_VOC_DATA_TYPE_G711A:
		case AUD_INTF_VOC_DATA_TYPE_G711U:
			rtc_bk_memcpy(temp_tx_info.ping.buff_addr, aud_tras_drv_info.voc_info.encoder_temp.law_data, temp_mic_samp_rate_points);
			break;

		case AUD_INTF_VOC_DATA_TYPE_PCM:
			rtc_bk_memcpy(temp_tx_info.ping.buff_addr, aud_tras_drv_info.voc_info.encoder_temp.pcm_data, temp_mic_samp_rate_points * 2);
			break;
...
static bk_err_t aud_tras_dec(void)
{
	...
	if (aud_tras_drv_info.voc_info.aec_enable) {
		...
		if ((mic_fill_size + speaker_fill_size - ref_fill_size)/2 < 0) {
			LOGE("MIC_DELAY is error, ref_fill_size: %d \r\n", ref_fill_size);
	...
	/* save the data after G711A processed to encoder_ring_buffer */
		...
		if (aud_tras_drv_info.voc_info.spk_type == AUD_INTF_SPK_TYPE_UAC) {
			...
			if (size != aud_tras_drv_info.voc_info.speaker_samp_rate_points*2) {
				LOGE("read one frame pcm speaker data fail \r\n");
				os_memset(aud_tras_drv_info.voc_info.uac_spk_buff, 0x00, aud_tras_drv_info.voc_info.speaker_samp_rate_points*2);
			}
		}
	}
...
static bk_err_t aud_tras_drv_spk_req_data(audio_packet_t * packet)
{
	...
	if (aud_tras_drv_info.spk_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
		...
		if (size != aud_tras_drv_info.spk_info.frame_size) {
			LOGE("[%s] write spk_data to audio dac fail, size:%d, line: %d \n", __func__, size, __LINE__);
			//TODO
			//add handle code
		}
#if 0
...
bk_err_t aud_tras_drv_spk_init(aud_intf_spk_config_t *spk_cfg)
{
	bk_err_t ret = BK_OK;
	...
	if (aud_tras_drv_info.spk_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
		...
		if(aud_tras_drv_info.spk_info.uac_spk_buff ==NULL){
			LOGE("malloc uac spk buffer fail \r\n");
			err = BK_ERR_AUD_INTF_MEMY;
			goto aud_tras_drv_spk_init_exit;
		}
...
bk_err_t aud_tras_drv_spk_deinit(void)
{
	if (aud_tras_drv_info.spk_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
		/* disable audio dac */
		bk_aud_dac_stop();
...
bk_err_t aud_tras_drv_spk_start(void)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
	uint32_t size = 0;
...
bk_err_t aud_tras_drv_spk_pause(void)
{
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
bk_err_t aud_tras_drv_spk_stop(void)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
bk_err_t aud_tras_drv_spk_set_chl(aud_intf_spk_chl_t spk_chl)
{
...
bk_err_t aud_tras_drv_mic_init(aud_intf_mic_config_t *mic_cfg)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
//	aud_tras_drv_info.mic_info.aud_tras_drv_mic_event_cb = mic_cfg->aud_tras_drv_mic_event_cb;
	...
	if (aud_tras_drv_info.mic_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		...
		if (aud_tras_drv_info.mic_info.adc_config == NULL) {
			LOGE("adc_config audio_tras_drv_malloc fail \r\n");
			err = BK_ERR_AUD_INTF_MEMY;
		...
		if (aud_tras_drv_info.mic_info.uac_mic_config == NULL) {
			...
			if (aud_tras_drv_info.mic_info.uac_mic_config == NULL) {
				LOGE("uac_config audio_tras_drv_malloc fail \r\n");
				err = BK_ERR_AUD_INTF_MEMY;
				goto aud_tras_drv_mic_init_exit;
			} else {
...
bk_err_t aud_tras_drv_mic_deinit(void)
{
	if (aud_tras_drv_info.mic_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		/* disable audio adc */
		bk_aud_adc_stop();
...
bk_err_t aud_tras_drv_mic_start(void)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
/* not stop dma, only disable adc */
bk_err_t aud_tras_drv_mic_pause(void)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
bk_err_t aud_tras_drv_mic_stop(void)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
bk_err_t aud_tras_drv_mic_set_chl(aud_intf_mic_chl_t mic_chl)
{
	bk_err_t ret = BK_ERR_AUD_INTF_OK;
	switch (mic_chl) {
		case AUD_INTF_MIC_CHL_MIC1:
...
bk_err_t aud_tras_drv_mic_set_samp_rate(uint32_t samp_rate)
{
	bk_err_t ret = BK_ERR_AUD_INTF_OK;
...
bk_err_t aud_tras_drv_voc_deinit(void)
{
//	if (aud_tras_drv_info.voc_info.status == AUD_TRAS_DRV_VOC_STA_NULL)
//		return BK_ERR_AUD_INTF_OK;
...
bk_err_t aud_tras_drv_voc_init(aud_intf_voc_config_t* voc_cfg)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
//	aud_tras_drv_info.voc_info.aud_tras_drv_voc_event_cb = voc_cfg->aud_tras_drv_voc_event_cb;
	...
	if (aud_tras_drv_info.voc_info.aec_enable) {
		...
		if (aud_tras_drv_info.voc_info.aec_info == NULL) {
			LOGE("aec_info audio_tras_drv_malloc fail \r\n");
			err = BK_ERR_AUD_INTF_MEMY;
			...
			if (aud_tras_drv_info.voc_info.aec_info->aec_config == NULL) {
				LOGE("aec_config_t audio_tras_drv_malloc fail \r\n");
				err = BK_ERR_AUD_INTF_MEMY;
				goto aud_tras_drv_voc_init_exit;
			} else {
	...
	LOGI("aud_tras_drv_info.voc_info.data_type:%d \r\n", aud_tras_drv_info.voc_info.data_type);
	...
	if (aud_tras_drv_info.voc_info.spk_type == AUD_INTF_SPK_TYPE_BOARD) {
		...
		if (aud_tras_drv_info.voc_info.adc_config == NULL) {
			LOGE("dac_config audio_tras_drv_malloc fail \r\n");
			err = BK_ERR_AUD_INTF_MEMY;
			goto aud_tras_drv_voc_init_exit;
		} else {
	...
	if (aud_tras_drv_info.voc_info.mic_type == AUD_INTF_MIC_TYPE_BOARD) {
		...
		LOGI("%s, uac mic_ring_buff:%p, size:%d \n", __func__, aud_tras_drv_info.voc_info.mic_ring_buff, aud_tras_drv_info.voc_info.mic_samp_rate_points*2*aud_tras_drv_info.voc_info.mic_frame_number + CONFIG_AUD_RING_BUFF_SAFE_INTERVAL);
...
bk_err_t aud_tras_drv_voc_start(void)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
	uint32_t size = 0;
...
bk_err_t aud_tras_drv_voc_stop(void)
{
	bk_err_t ret = BK_OK;
...
bk_err_t aud_tras_drv_voc_ctrl_mic(aud_intf_voc_mic_ctrl_t mic_en)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
...
bk_err_t aud_tras_drv_voc_ctrl_spk(aud_intf_voc_spk_ctrl_t spk_en)
{
	bk_err_t ret = BK_OK;
	bk_err_t err = BK_ERR_AUD_INTF_FAIL;
	uint32_t size = 0;
...
bk_err_t aud_tras_drv_voc_ctrl_aec(bool aec_en)
{
	GLOBAL_INT_DECLARATION();
...
bk_err_t aud_tras_drv_set_aec_para(aud_intf_voc_aec_ctl_t *aec_ctl)
{
	switch (aec_ctl->op) {
...
bk_err_t aud_tras_drv_get_aec_para(void)
{
...
bk_err_t aud_tras_drv_voc_tx_debug(aud_intf_dump_data_callback callback)
{
	GLOBAL_INT_DECLARATION();
...
bk_err_t aud_tras_drv_voc_rx_debug(aud_intf_dump_data_callback callback)
{
	GLOBAL_INT_DECLARATION();
...
bk_err_t aud_tras_drv_voc_aec_debug(aud_intf_dump_data_callback callback)
{
	GLOBAL_INT_DECLARATION();
...
bk_err_t aud_tras_drv_uac_register_connect_state_cb(void * cb)
{
	GLOBAL_INT_DECLARATION();
...
bk_err_t aud_tras_drv_spk_set_samp_rate(uint32_t samp_rate)
{
	bk_err_t ret = BK_ERR_AUD_INTF_OK;
...
bk_err_t aud_tras_drv_set_mic_gain(uint8_t value)
{
	bk_err_t ret = BK_ERR_AUD_INTF_OK;
...
bk_err_t aud_tras_drv_set_spk_gain(uint16_t value)
{
	bk_err_t ret = BK_OK;
...
bk_err_t aud_tras_drv_init(aud_intf_drv_config_t *setup_cfg)
{
	...
	if ((!aud_trs_drv_thread_hdl) && (!aud_trs_drv_int_msg_que)) {
		...
		rtc_bk_memcpy(&aud_trs_drv_setup_bak, setup_cfg, sizeof(aud_intf_drv_config_t));
