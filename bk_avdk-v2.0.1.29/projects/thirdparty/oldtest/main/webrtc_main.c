...
const uint8 gc0328c_night_mode_talbe[][2] =
	{
		{0xfe, 0x00},
		{0xd1, 0x00},
		{0xd2, 0x00},
...
const uint8 gc0328c_day_mode_talbe[][2] =
	{
		{0xfe, 0x00},
		{0xd1, 0x19},
		{0xd2, 0x19},
...
const uint8 gc2145_night_mode_talbe[][2] =
	{
		{0xfe, 0x02},
		{0xd1, 0x00},
		{0xd2, 0x00},
...
const uint8 gc2145_day_mode_talbe[][2] =
	{
		{0xfe, 0x02},
		{0xd1, 0x32},
		{0xd2, 0x32},
...
void webrtc_mailbox_send_reboot_msg(void);	// 加的+++++++++
void webrtc_mailbox_send_netfail_msg(void); // 加的+++++++++
void webrtc_shutdown_cpu1();
void webrtc_network_configure_update();
static void white_light_on(void);  // 加的++++++++
static void white_light_off(void); // 加的+++++++++
...
{
	bk_printf("wwdadawdaadwadawdadaderrrrrrrrrrrrrrrrrr\n");
	...
	{
		wifi_connected = 0;
		bk_printf("password wrong1!!!\n");
		webrtc_mailbox_send_netfail_msg(); // 这里
		//+++++++++++++++++++
		LOGW("密码错误1!!!!\r\n");
	}
	break;
	...
	case WIFI_LINKSTATE_STA_DISCONNECTED:
	{
		...
		// webrtc_mailbox_send_netfail_msg();//这里2
		//+++++++++++++++++++
...
void camera_cfg_night_mode(bool is_nightmode)
{
	...
	{
	case SENSER_GC0328C:
		dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), 0xfe, 0x00);

		if (is_nightmode == 1)
		{
			size = sizeof(gc0328c_night_mode_talbe) / 2;
			for (i = 0; i < size; i++)
			{
				addr = gc0328c_night_mode_talbe[i][0];
				data = gc0328c_night_mode_talbe[i][1];
				dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), addr, data);
			}
			LOGW("GC0328C night_mode\r\n");
		}
		else
		{
			size = sizeof(gc0328c_day_mode_talbe) / 2;
			for (i = 0; i < size; i++)
			{
				addr = gc0328c_day_mode_talbe[i][0];
				data = gc0328c_day_mode_talbe[i][1];
				dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), addr, data);
			}
			LOGW("GC0328C day_mode\r\n");
		}

		break;
	case SENSER_GC2145:
		dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), 0xfe, 0x00);

		if (is_nightmode == 1)
		{

			size = sizeof(gc2145_night_mode_talbe) / 2;
			for (i = 0; i < size; i++)
			{
				addr = gc2145_night_mode_talbe[i][0];
				data = gc2145_night_mode_talbe[i][1];
				dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), addr, data);
			}
			LOGW("GC2145 night_mode\r\n");
		}
		else
		{
			size = sizeof(gc2145_day_mode_talbe) / 2;
			for (i = 0; i < size; i++)
			{
				addr = gc2145_day_mode_talbe[i][0];
				data = gc2145_day_mode_talbe[i][1];
				dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), addr, data);
			}
			LOGW("GC2145 day_mode\r\n");
		}
...
void camera_flip_contrl(uint8_t mirror_h, uint8_t mirror_v)
{
	...
	case SENSER_GC0328C:
		...
		if (mirror == 0 && vertical == 0)
		{
			dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), 0x17, GC0328_FLIP_INIT);
		}
		else if (mirror == 1 && vertical == 0)
		{

			dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), 0x17, GC0328_FLIP_INIT ^ 0x01);
		}
		else if (mirror == 0 && vertical == 1)
		{

			dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), 0x17, GC0328_FLIP_INIT ^ 0x02);
		}
		else if (mirror == 1 && vertical == 1)
		{

			dvp_camera_i2c_write_uint8((GC0328_WRITE_ADDRESS >> 1), 0x17, GC0328_FLIP_INIT ^ 0x03);
		}
		else
		{
			LOGW("flip set err\n");
	...
	case SENSER_GC2145:
		...
		if (mirror == 0 && vertical == 0)
		{
			dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), 0x17, GC2145_FLIP_INIT);
		}
		else if (mirror == 1 && vertical == 0)
		{

			dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), 0x17, GC2145_FLIP_INIT ^ 0x01);
		}
		else if (mirror == 0 && vertical == 1)
		{

			dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), 0x17, GC2145_FLIP_INIT ^ 0x02);
		}
		else if (mirror == 1 && vertical == 1)
		{

			dvp_camera_i2c_write_uint8((GC2145_WRITE_ADDRESS >> 1), 0x17, GC2145_FLIP_INIT ^ 0x03);
		}
		else
		{
			LOGW("flip set err\n");
...
void webrtc_mailbox_send_netfail_msg(void)
{
	LOGD("%s %d  \n", __func__, __LINE__);
	webrtc_cmd_t webrtc_cmd = {0};
	webrtc_cmd_t *pcmd = &webrtc_cmd;
...
// 初始化多个互斥锁和信号量来控制对共享资源的访问和同步不同线程的执行
	...
	rtos_init_semaphore_ex(&CPU0_can_change_reboot_sem, 1, 0); // 加的++++++++++++++++++++++++++
...
void webrtc_media_thread(void *param)
{
	...
	// 加的+++  key_reboot是为了只有按键重启才会触发这个循环,避开串口或其他错误导致的重启卡在这里
	/* 	while (key_reboot && media_yuv_thread_closed == true)
		{
			//此时yuv线程已经跑完
			webrtc_mailbox_cpu1_to_cpu0_msg();//告诉cpu0释放信号量继续按键重启流程
			bk_printf("yuv和media已跑完且已发送消息给cpu0\n");
			break;
...
{
	LOGW("%s %d \n", __func__, __LINE__);
	bk_printf("webrtc_audio_play_wavfile:3885--------------------\n");
	int wavfd = -1;
	size_t size;
	bk_err_t ret = BK_OK;
	char szwavfile[64];
	...
	if (wavfd >= 0)
	{
		bk_printf("webrtc_audio_play_wavfile:3916 open wavfile success\n");
		...
		if (size > 0)
		{
			...
			*/
			bk_printf("webrtc_audio_play_wavfile:3932 read size > 0\n");
	...
	{
		bk_printf("webrtc_audio_play_wavfile:4005 open wavfile fail\n");
	...
	bk_printf("webrtc_audio_play_wavfile:4020  play_file_index = -1 and false \n");
	play_file_index = -1;
	webrtc_audio_playing_file = false; // 播放完毕指示没有音频文件正在播放

	// 加的+++++++++++++++++++++++    播放完毕告诉CPU0释放信号量  只有播放的是reboot.wav才能进来 play_file_index ==3
...
void webrtc_audio_play_thread(void *param)
{
	...
	while (webrtc_audio_play_runing)
	{
		bk_printf("ready to get webrtc_audio_play_sem666666666 \n");
...
void webrtc_audio_play()
{
	if (webrtc_audio_play_sem != NULL)
	{
		...
		bk_printf("&webrtc_audio_play_sem count = %d\n", count);
		if (count == 0)
		{
...
void webrtc_handle_req_rx(webrtc_cmd_t *pcmd)
{
	...
	else if (pcmd->mb_cmd.param1 == APP_COM_WIFI_CONNECT)
	{
		...
		if (webrtc_streamer_runing == false && webrtc_streamer_start_cmd == false)
		{
			...
			bk_printf("APP_COM_WIFI_CONNECT**********************webrtc_audio_playing_init == %d audio_opened == %d ,runSystemCfg.deviceInfo.playSound == %d\n", webrtc_audio_playing_init, audio_opened, runSystemCfg.deviceInfo.playSound);
			if (audio_opened == true)
			{

				// 播放连接WIFI音频文件
				rtos_lock_mutex(&play_file_index_mutex);
				bk_printf(" lock play index == 4!!!!\n");
				play_file_index = 4;
				webrtc_audio_play();
				rtos_unlock_mutex(&play_file_index_mutex);
				bk_printf(" unlock play index == 4 wifiok!!!!\n");
	...
	{
#if CONFIG_SYS_CPU1
		bk_printf("APP_COM_REBOOT*****webrtc_audio_playing_init == %d audio_opened == %d ,runSystemCfg.deviceInfo.playSound == %d\n", webrtc_audio_playing_init, audio_opened, runSystemCfg.deviceInfo.playSound);
		if (audio_opened == true)
		{
			...
			// 播放reboot.wav音频文件
			rtos_lock_mutex(&play_file_index_mutex);
			bk_printf(" lock play index == 3!!!!\n");
			play_file_index = 3;
			webrtc_audio_play();
			rtos_unlock_mutex(&play_file_index_mutex);
			bk_printf(" unlock play index == 3 reboot!!!!\n");
			bk_printf("play_file_index = %d;+++++++++++++++++++++++++++\n", play_file_index);
		}
	...
	{
#if CONFIG_SYS_CPU1
		bk_printf("APP_COM_NETFAIL*****webrtc_audio_playing_init == %d audio_opened == %d ,runSystemCfg.deviceInfo.playSound == %d\n", webrtc_audio_playing_init, audio_opened, runSystemCfg.deviceInfo.playSound);
		if (audio_opened == true)
		{
			// 播放netfail.wav音频文件
			rtos_lock_mutex(&play_file_index_mutex);
			bk_printf(" lock play index == 2!!!!\n");
			play_file_index = 2;
			webrtc_audio_play();
			rtos_unlock_mutex(&play_file_index_mutex);
			bk_printf(" unlock play index == 2 netfail!!!!\n");
			bk_printf("play_file_index = %d;+++++++++++++++++++++++++++\n", play_file_index);
		}
	...
	{
#if CONFIG_SYS_CPU1
		if (webrtc_media_thread_runing != false) // 这玩意media和yuv线程的while都要用到
		{
			webrtc_media_thread_runing = false;
	...
	else if (pcmd->mb_cmd.param1 == APP_COM_CHANGE_SEM)
	{
		...
		if (CPU0_can_change_reboot_sem != NULL)
		{
			...
			bk_printf("CPU0 ready to set sem!!!!********\n");
			if (count == 0)
			{
...
void webrtc_get_camera_info(cJSON *response_msg_resp, cJSON *item)
{
	...
	cJSON_AddNumberToObject(jpreview, "mirror_h", mirror_h);								// 水平翻转
	cJSON_AddNumberToObject(jpreview, "mirror_v", mirror_v);								// 垂直翻转
...
void webrtc_get_led_info(cJSON *response_msg_resp, cJSON *item)
{
	...
	if (LED_open == 1)
	{
		...
		cJSON_AddNumberToObject(jrespgdi, "led_on", LED_open);
...
void webrtc_set_led_on(cJSON *response_msg_resp, cJSON *item)
{
	/* 	cJSON *jresps = cJSON_CreateObject();
		...
		cJSON_AddItemToArray(response_msg_resp,jrespgdi); */
	cJSON *jresps = cJSON_CreateObject();
	cJSON *jrespgdi = cJSON_CreateObject();
...
void webrtc_set_record(cJSON *response_msg_resp, cJSON *item)
{
	...
	LOGW("44444444444444444444444444444444444\n");
	...
	if (set_record_auto != NULL)
	{
		...
		LOGW("webrtc_record_auto:%d\n", webrtc_record_auto);
		runMyRecordCfg.enable = webrtc_record_auto;
...
void webrtc_set_roi_config(cJSON *response_msg_resp, cJSON *item)
{
	...
	LOGW("111111111111111111111111111111111111111111111111\n");
	cJSON_AddNumberToObject(jrespgdi, "code", 200);
	...
	webrtc_cpu1_cpu2_roi_set(roi_whole_region, roi_start_x, roi_start_y, roi_end_x, roi_end_y);
...
void thread_Download(void *arg)
{
	...
	printf("thread_DownLoad = %s delay= %d\n", dl->szsessionId, delay);
	char *psendbuf = NULL;
	int sendmsg = 0;
	int sendsize = 0;
	uint16_t index = 0;
	...
	if (dl != NULL && pdbuf != NULL)
	{
		...
		while (!dl->stop_ && runhua_runing)
		{
			...
			if (dl->fp > 0 && dl->stop_ == false && dl->started == true && dl->end_ == false)
			{
				...
				if (now - dl->send_state_time > 1000)
				{
					bk_printf("=====================now-dl->send_state_time>1000==================================\n");

					dl->send_state_time = now;
					printf("webrtc download  %d %%  %u   %d\n", 100 * sended / dl->nFileLen, now - dl->starttime, sendrate);
					sendrate = 0;
				...
				else
				{
					...
					if (readed > 0)
					{
						...
						while (!dl->stop_ && runhua_runing)
						{
							...
							{
								dl->end_ = true;
								printf("webrtc download failed send datachannel message (WEBRTC_DMT_BINARY) %d\n", ret);
								break;
							}
						}
					}
	...
	if (root != NULL)
	{
		...
		if (jeventName != NULL && jdata != NULL)
		{
			...
			if (jmessage != NULL)
			{
					...
					if (dl == NULL)
					{
						...
						{ // 限制最大下载数
							...
							if (dl != NULL)
							{
								...
								// printf("webrtc_stream_datachannel_message_callback pthread_create \n");
				...
				{	
					LOGW("222222222222222222222222222222222222222222\n");
...
void webrtc_streamer_start(void)
{
	...
	if (webrtc_streamer_runing == false)
	{
		...
		webrtc_streamer_set_max_channel(3); // 最大会话数
...
static void night_sensor_adc_config(adc_chan_t chan)
{
	...
	config.chan = chan;
	config.adc_mode = ADC_CONTINUOUS_MODE; // 原来是 ADC_CONTINUOUS_MODE连续转换
	config.src_clk = ADC_SCLK_XTAL_26M;
	config.clk = 3203125; // 原来是3203125   0x30e035
	...
	{
		adc_isinited = true;
		BK_LOG_ON_ERR(bk_adc_init(chan)); // 这里导致gpio busy
		// LOGW("%s %d \n", __func__, __LINE__);
...
#if CONFIG_WEBRTC_AVI
// WebRTC录制线程   处理视频和音频数据的录制，并将其写入AVI文件
void webrtc_record_thread(void *param)
{
	bk_err_t ret = BK_OK;
	webrtc_avi_t *avi_writer = NULL; // 用于处理AVI文件的写入
	webrtc_record_runing = true;
	char szfilename[128] = {0};
	char szfilepath[128] = {0};
	bool change_file = false;
	bool can_write = false;
	bool is_write_audio = false; // 音频写入状态
	bool queue_empty = false;
	char stime[32] = {0};							// 存储时间戳
	char sdate[32] = {0};							// 存储日期
	uint32_t start_timestamp = get_cur_timestamp(); // 录制开始时间戳
	uint32_t starttime;
	uint32_t stoptime;
	uint32_t now;					// 当前时间
	uint32_t max_delay = 60 * 1000; // ms
	uint32_t usetime;				// 计算时间差和延迟
	uint32_t delay;					// 计算时间差和延迟
	int result = 0;					// 保存函数调用的结果
	...
	while (webrtc_record_runing && webrtc_record_sem != NULL)
	{
			...
			if (pdata != NULL)
			{
				...
				if (pdata != NULL)
				{
					...
					if (webrtc_recording && sdcard_mounted == 1)
					{
						...
						// 如果允许写入且avi_writer存在，则写入视频和音频数据
		...
		if (webrtc_recording == false && avi_writer != NULL)
		{
			if (avi_writer != NULL)
			{
				...
				stoptime = get_cur_timestamp();
				LOGW("%s %d stop write avi file %s \n", __func__, __LINE__, szfilename);
				bk_printf("timeused = %dms %ds\n", (stoptime - starttime), (stoptime - starttime) / 1000);
				rtos_lock_mutex(&webrtc_record_queue_mutex);
				webrtc_can_record = false;
				rtos_unlock_mutex(&webrtc_record_queue_mutex);
				WEBRTC_AVI_close(avi_writer);
...
#if 0	
	//++++ 注释信号量
    webrtc_mailbox_cpu1_to_cpu0_msg();//发给cpu0继续下一步reboot操作-->第二次get信号量成功
	//bk_printf("已告诉cpu0继续reboot\n");
#endif
...
void webrtc_network_configure_update()
{
	...
	webrtc_http_get_net_config(szurl); // 发送请求给服务器
}
// 获取设备信息(通过发送http 请求)
...
void webrtc_network_configure_callback(char *ssid, char *password, char *key, char *name)
{
	...
	if (name != NULL)
	{
		snprintf(config_wifi_name, 64, "%s", name); // 复制Wi-Fi名称
	}
	else
	{
		snprintf(config_wifi_name, 64, "%s", "");
	}
	configure_wifi_update = true;		 // 表明Wi-Fi配置已更改
	runNetworkCfg.wireless.isConfig = 0; //
	// 复制SSID和密码到运行时配置结构
	snprintf((char *)runNetworkCfg.wireless.essid, 32, "%s", ssid);
...
void webrtc_read_system_info()
{
		...
		if(pt!= NULL){					
				...
				if(ret== BK_OK){
					...
					if(sys_info_data.head == SYSTEM_INFO_HEAD){
						...
						if(strcmp(sys_info_data.serialNumber,runSystemCfg.deviceInfo.serialNumber)==0){
							bk_printf("strcmp(sys_info_data.serialNumber,runSystemCfg.deviceInfo.serialNumber) == 0,没改到序列号\n");
						}else{
							...
							bk_printf("read_systemn_info修改成功!\n");
						    SystemCfgSave();
						}
	...
	snprintf(runSystemCfg.deviceInfo.serialNumber, 64, "%s", "RHZL-00-FDX3-085E-00003055");
}
#endif
void runhua_main(void)
{
	...
	rtos_init_mutex(&play_file_index_mutex); // 加的+++++++++++++++
#endif
#if CONFIG_SYS_CPU0
	int check_adc_count = 0;
	...
	// WIFI  runNetworkCfg.wireless.isConfig ==1为需要配置 或 wifi的ssid和密码为空
	...
	{
		system_run_state = SYSTEM_WIFI_CONFIG; // 设置系统运行状态
	...
	// 启动webrtc音频流的初始化,音频接口的初始化、配置和启动
	webrtc_stream_start_audio_start();
	// 初始化webrtc运动检测模块,为运动检测模块分配必要的内存资源
	webrtc_motion_init();
#endif
	...
	while (runhua_runing)
	{
		...
		LOGW("=========motion_detection is %d===========\n",motion_detection);
		// LOGW("==%d,%d,%d,%d,%d==",roi_whole_region,roi_start_x,roi_start_y,roi_end_x,roi_end_y);
		...
		{
			print_num = 'b';
			bk_printf("runSystemCfg.deviceInfo.playSound = %d+++++++++\n", runSystemCfg.deviceInfo.playSound); //+++++++++++
		}

		// 如果音频设备已打开且设备要播放声音，但是音频播放还没初始化  	//&& runSystemCfg.deviceInfo.playSound == 1
		if (webrtc_audio_playing_init == false && audio_opened == true)
		{
			webrtc_audio_playing_init = true;
			bk_printf(" ready to play!!!!\n");
			if (runNetworkCfg.wireless.isConfig == 0)
			{
				rtos_lock_mutex(&play_file_index_mutex);
				play_file_index = 1;
				bk_printf(" lock play index == 1!!!!\n");
				// rtos_unlock_mutex(&play_file_index_mutex);
			}
			else
			{
				rtos_lock_mutex(&play_file_index_mutex);
				play_file_index = 0;
				bk_printf(" lock play index == 0!!!!\n");
				// rtos_unlock_mutex(&play_file_index_mutex);
			}

			webrtc_audio_play();
			rtos_unlock_mutex(&play_file_index_mutex);
			bk_printf("goto play index == %d\n", play_file_index);
		}
		// 如果流媒体服务未运行但启动命令已发出，则启动流媒体服务并初始化NTP同步
		...
		// 音频数据上锁，遍历音频数据列表，处理每个音频数据，然后移除并销毁
		rtos_lock_mutex(&audio_mutex);
		...
		if (now - check_adc_timestamp >= 1000)
		{
			...
			if (cpu1_runing == true)
			{
				uint32_t adcdata = adc_get_value(15); // 15--13busy
		...
		default_key_down_value = bk_gpio_get_value(KEY_DEFAULT); // KEY_DEFAULT是枚举，其中按键为44
			...
			else
			{
				...
				if (default_key_down_timestamp != 0)
				{
					...
					if (default_delay >= 4000)
					{
						...
						//++++++++++++
						bk_printf("rtos_get_semaphore CPU0_can_change_reboot_sem OK!!!\n");
...
#if 0
							//cpu0发个消息告诉cpu1的录制线程停止
							webrtc_mailbox_send_stop_record_msg();//加的++++++++++

							/*cpu1录制线程停止后，告诉cpu0，这里下面的步骤可以进行了
							然后cpu0的消息处理函数给这里下面释放信号量*/
						
							//在这里获取信号量,有了之后下面的全局变量可以改变了，进而关闭cpu和重启
							rtos_get_semaphore(&CPU0_can_change_reboot_sem,1000);//加的++++++++++


							/*	这里轮到停止dvp
								cpu0发个消息告诉cpu1的dvp线程停止-->webrtc_media_thread
								shit,还有media_yuv_thread,那就在yuv线程里面跑完后修改一个全局变量,
								media里while等待这个全局变量被修改再
								发送信息给cpu0，让它释放CPU0_can_change_reboot_sem	
							*/
							webrtc_mailbox_send_stop_media_msg();//加的++++++++++
		...
		// 当Wi-Fi连接成功且系统信息初始化完成时，向CPU1发送Wi-Fi连接成功的消息，可能先清理蓝牙模块(如果定义了相应的配置)
...
// CPU0卸载SD卡 发送消息给CPU1令其停止 关闭cpu1 恢复cpu1邮件通信到初始状态 停止MDNS服务
	...
	rtos_deinit_mutex(&play_file_index_mutex); // 加的++++++++++++++++++++++++++++
...
void webrtc_run()
{
	...
	// 初始化 webrtc退出记录 信号量  初始值为0 最大值为1
	...
	// 加的++++++++++++++++++++++++++++++
	ret = rtos_init_semaphore_ex(&CPU0_can_change_reboot_sem, 1, 0);
	if (ret != BK_OK)
	{
		LOGE("create rtos_init_semaphore_ex fail\n");
	}
