...
int32_t dm_gattc_connect(uint8_t *addr, uint32_t addr_type)
{
    ...
    if (0 == dm_gatt_find_id_info_by_nominal_info(addr, addr_type, peer_id_addr, &peer_id_addr_type) && g_dm_gap_use_rpa)
    {
        gatt_logi("local use rpa");
        param.local_addr_type = BLE_ADDR_TYPE_RPA_PUBLIC;
...
int dm_gattc_main(void)
{
    ...
    bk_bd_addr_t public_addr;
    char adv_name[64] = {0};

    bk_get_mac((uint8_t *)public_addr, MAC_TYPE_BLUETOOTH);

    for (int i = 0; i < sizeof(public_addr) / 2; i++)
    {
        uint8_t tmp_addr = public_addr[i];
        public_addr[i] = public_addr[sizeof(public_addr) - 1 - i];
        public_addr[sizeof(public_addr) - 1 - i] = tmp_addr;
    }

    //public_addr[0]++;
    public_addr[5] |= 0xc0; // static random addr[47:46] must be 0b11 in msb !!!

    snprintf((char *)(adv_name), sizeof(adv_name) - 1, "CENTRAL-%02X%02X%02X", public_addr[2], public_addr[1], public_addr[0]);
