...
调试步骤如下：
	...
	- 3、首先发送串口指令 ``aud_intf_set_aec_param_test ec_depth {value}`` 来设置 ``echo depth`` , 回声越大值设置越大, 当继续增大该值, 但是无法提升回声消除效果时停止设置该值;
	...
	- 5、然后根据听到的底噪大小发送串口指令 ``aud_intf_set_aec_param_test ns_level {value}`` 来设置 ``noise level`` 底噪越大，设置的值越大, 优化底噪消除效果;
