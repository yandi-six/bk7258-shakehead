...
off_t bk_vfs_tell(int fd){
	...
	if (file->f_ops && file->f_ops->tell) {
		ret = file->f_ops->tell(file);
	} else {
		bk_set_errno(ENOTSUP);
	}
...
int bk_vfs_rename(const char *oldpath, const char *newpath) {
	...
	if (!fs_old->f_ops->rename) {
		bk_set_errno(ENOTSUP);ret = -4;
		goto out;
	}

	full_path_new = bk_normalize_path(newpath);
	if (!full_path_new) {ret = -5;
		goto out;
	}
