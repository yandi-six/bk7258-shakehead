...
extern volatile int download_process;
extern void webrtc_mailbox_send_ota_process(int ota_process);
extern beken_semaphore_t ota_can_continute_sem;
int bk_http_client_get_data(bk_http_client_handle_t client)
{
	...
	if(!client->response->is_chunked){
		...
		download_process = client->response->data_process*100 / client->response->content_length;
		
		//webrtc_mailbox_send_ota_process(download_process);
		//rtos_get_semaphore(ota_can_continute_sem, 500);//session close can continue ota
		//os_printf("data_process=%d, content_length=%d  %d\r\n", client->response->data_process, client->response->content_length,download_process);
	}
	else{
		...
		//webrtc_mailbox_send_ota_process(download_process);
		//rtos_get_semaphore(ota_can_continute_sem, 500);//session close can continue ota
		//os_printf("is_chunked, download_process=%d\r\n", download_process);
 ...
 * @brief 执行HTTP客户端请求
 * 
 * 该函数负责驱动HTTP客户端状态机，以执行HTTP请求的各个步骤
 * 它会根据客户端的状态来连接服务器、发送请求、传输数据、接收响应等
 * 
 * @param client HTTP客户端句柄，包含执行请求所需的所有信息
 * @return bk_err_t 返回错误代码，表示请求执行的状态
 */
bk_err_t bk_http_client_perform(bk_http_client_handle_t client)
{
    bk_err_t err;
    do {
        // 如果客户端标记了需要重新处理，则重新准备HTTP客户端
        ...
        switch (client->state) {
            case HTTP_STATE_INIT:
                ...
                // 注意：这里没有break，是为了在成功连接后直接进入下一个状态
            ...
            case HTTP_STATE_CONNECTED:
                ...
                // 同样没有break，以连续执行请求发送后的操作

            case HTTP_STATE_REQ_COMPLETE_HEADER:
                // 请求完成头部发送状态，开始发送POST数据（如果有）
            ...
            case HTTP_STATE_RES_ON_DATA_START:
                ...
                while (client->response->is_chunked && !client->is_chunk_complete) {
                    os_printf(TAG, "begin get data, is_chunked TRUE\r\n");
...
{
	os_printf("https_test_download_task\n");
	int err;
