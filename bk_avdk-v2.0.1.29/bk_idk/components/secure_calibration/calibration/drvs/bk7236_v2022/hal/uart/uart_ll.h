...
#if CONFIG_FPGA
		baudrate1 = UART_CLOCK_FREQ_FPGA / (hw1->config.clk_div + 1);
		baudrate2 = UART_CLOCK_FREQ_FPGA / (hw2->config.clk_div + 1);
#else
	if (AON_PMU_IS_XTAL_SEL_40M) {
		baudrate1 = UART_CLOCK_FREQ_40M / (hw1->config.clk_div + 1);
		baudrate2 = UART_CLOCK_FREQ_40M / (hw2->config.clk_div + 1);
	} else {
		baudrate1 = UART_CLOCK_FREQ_26M / (hw1->config.clk_div + 1);
		baudrate2 = UART_CLOCK_FREQ_26M / (hw2->config.clk_div + 1);
	}
#endif
