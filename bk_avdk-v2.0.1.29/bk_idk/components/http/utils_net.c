...
int32_t HAL_TCP_Read(uintptr_t fd, char *buf, uint32_t len, uint32_t timeout_ms)
{
	...
	do
	{
		...
		rtos_delay_milliseconds(2); // jiade+++
	...
	if (((((float)(len_recv)) / ((float)(bk_http_ptr->http_total))) * 100 == 100) && ((((float)(len_recv)) / ((float)(bk_http_ptr->http_total))) * 100 > 90))
	{
		...
		bk_ptr = bk_flash_partition_get_info(BK_PARTITION_OTA); // note: when update_partition is B, arg: BK_PARTITION_APPLICATION1,update_partition is A��arg: BK_PARTITION_APPLICATION
