...
int http_data_process(char *buf, UINT32 len, UINT32 recived, UINT32 total)
{
	...
	os_printf("idk==============cyg_recvlen_per:(%.2f)%%\r\n",(((float)(recived))/(total))*100);

	cyg_recvlen_per = ((float)recived / total) * 100;//加的+++
	
	//os_printf("cyg_recvlen_per:%f\n",cyg_recvlen_per);
	//1.0
	...
	if (now - start_timestamp >= 2000 || cyg_recvlen_per == 100)
	{
		if (cyg_recvlen_per < 100)
		{
			webrtc_mailbox_send_ota_process(cyg_recvlen_per);
			rtos_get_semaphore(ota_can_continute_sem,1000);
		}else if(cyg_recvlen_per == 100 && sendonce == 0){
	...
	//2.0
	//rtos_get_semaphore(ota_process_sem,BEKEN_NEVER_TIMEOUT);//cpu0那个
					  ...
					  httpclient_data_t *client_data)
{
	...
	do {
		...
		if (0 == client->net.handle) {
			...
			iotx_net_init(&client->net, host, url_port, ca_crt);//进度在里面
