...
static uint32_t now;//当前时间
static uint32_t start_timestamp;
static bool run_once = true;
extern int sendonce;
extern beken_semaphore_t ota_process_sem;//加的+++
extern beken_semaphore_t ota_can_continute_sem;//加的+++
extern volatile float cyg_recvlen_per; // 用于存储进度百分比 cpu0的
extern void webrtc_mailbox_send_ota_process(float ota_process);//+++
extern uint32_t get_cur_timestamp(void);
