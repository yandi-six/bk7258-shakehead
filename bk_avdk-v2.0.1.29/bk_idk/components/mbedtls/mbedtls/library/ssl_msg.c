 ...
 * 准备接收到的握手记录以进行处理。
 *
 * 此函数验证并准备接收到的握手消息，以便进一步处理。
 * 它检查消息长度，并且对于 DTLS，它还验证握手头和序列号，确保消息是预期的且未乱序。
 *
 * @param ssl 包含 SSL 连接必要信息的 SSL 上下文结构。
 * @return 成功返回 0，或特定错误代码。
 */
int mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context *ssl)
{
    // 检查消息长度是否小于握手头长度
    if (ssl->in_msglen < mbedtls_ssl_hs_hdr_len(ssl)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("握手消息太短: %" MBEDTLS_PRINTF_SIZET,
                                  ssl->in_msglen));
        return MBEDTLS_ERR_SSL_INVALID_RECORD;
    }
   ...
   // os_printf("%d  %d\n",mbedtls_ssl_hs_hdr_len(ssl),ssl_get_hs_total_len(ssl));
    // 调试日志记录握手消息的详细信息
    MBEDTLS_SSL_DEBUG_MSG(3, ("握手消息: msglen ="
                              " %" MBEDTLS_PRINTF_SIZET ", type = %u, hslen = %"
                              MBEDTLS_PRINTF_SIZET,
                              ssl->in_msglen, ssl->in_msg[0], ssl->in_hslen));

#if defined(MBEDTLS_SSL_PROTO_DTLS)
//  os_printf("握手消息: msglen ="
//                               " %" MBEDTLS_PRINTF_SIZET ", type = %u, hslen = %"
//                               MBEDTLS_PRINTF_SIZET,
//                               ssl->in_msglen, ssl->in_msg[0], ssl->in_hslen);
//os_printf("12aaaaaaaaaaaaaa\n");
    // 对于 DTLS，执行握手消息的额外检查
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        ...
        if (ssl_check_hs_header(ssl) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("无效的握手头"));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
        ...
        if (ssl->handshake != NULL &&
            ...
            if (recv_msg_seq > ssl->handshake->in_msg_seq) {
                MBEDTLS_SSL_DEBUG_MSG(2,
                                      (
                                          "收到未来的握手消息，序列号为 %u (下一个 %u)",
                                          recv_msg_seq,
                                          ssl->handshake->in_msg_seq));
                return MBEDTLS_ERR_SSL_EARLY_MESSAGE;
            }

            // 如果适用，重新发送上一个飞行中的最后一条消息
            if (recv_msg_seq == ssl->handshake->in_flight_start_seq - 1 &&
                ssl->in_msg[0] != MBEDTLS_SSL_HS_HELLO_VERIFY_REQUEST) {
                MBEDTLS_SSL_DEBUG_MSG(2, ("收到上一个飞行中的消息，"
                ...
                MBEDTLS_SSL_DEBUG_MSG(2, ("丢弃乱序的消息: "
                                          "message_seq = %u, 预期 = %u",
                                          recv_msg_seq,
                                          ssl->handshake->in_msg_seq));
            }
        ...
        /* 等待消息完成后再递增 in_msg_seq */

        /* 处理消息重组和未来消息的缓冲 */
        if (ssl_hs_is_proper_fragment(ssl) == 1) {
            MBEDTLS_SSL_DEBUG_MSG(2, ("发现分段的 DTLS 握手消息"));
            return MBEDTLS_ERR_SSL_EARLY_MESSAGE;
        }
    } else
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    /* 对于 TLS，当前不处理分段 */
    /* Handle message reassembly and buffering of future messages */
    if (ssl->in_msglen < ssl->in_hslen) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("TLS 握手分段不支持"));
 ...
 * 读取并处理一个SSL/TLS记录。
 *
 * 此函数负责从SSL/TLS连接中读取记录，处理记录的消息类型，并在必要时更新握手状态。
 *
 * @param ssl 指向SSL上下文的指针。
 * @param update_hs_digest 表示是否更新握手摘要的标志。
 * @return 成功返回0，或特定的错误代码。
 *
 * 注意：
 * - 函数检查当前消息是否应保留。如果不应保留，则尝试消耗当前消息。
 * - 对于DTLS，函数检查缓冲消息并相应地处理它们。
 * - 函数继续处理，直到遇到非致命错误或成功完成。
 */
...
//os_printf("%d   ret:%#x\n", __LINE__,ret); // 打印调试信息
    ...
    if (ssl->keep_current_message == 0) {
        do {
            ...
            // 处理接收到的记录的消息类型
            ret = mbedtls_ssl_handle_message_type(ssl);//os_printf("%d   ret:%d\n", __LINE__,ret); // 打印调试信息
        ...
        // 如果接收到的是握手消息且需要更新握手摘要，则更新握手状态
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
            update_hs_digest == 1) {
            mbedtls_ssl_update_handshake_status(ssl);
        }
    } else {
        MBEDTLS_SSL_DEBUG_MSG(2, ("重用之前读取的消息"));
 ...
 * 处理接收到的SSL/TLS记录消息类型
 *
 * 此函数根据消息类型处理不同的SSL/TLS记录消息，例如握手消息、更改密码规范消息和警报消息。
 * 它确保消息的有效性，并调用适当的处理函数或直接处理这些消息。
 *
 * @param ssl 包含SSL/TLS会话所需信息的SSL/TLS上下文结构。
 * @return 成功时返回0，失败时返回特定的错误代码。
 */
int mbedtls_ssl_handle_message_type(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC) {
        if (ssl->in_msglen != 1) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("无效的CCS消息，长度: %" MBEDTLS_PRINTF_SIZET,
                                      ssl->in_msglen));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }

        if (ssl->in_msg[0] != 1) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("无效的CCS消息，内容: %02x",
                                      ssl->in_msg[0]));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
            ...
            ssl->state != MBEDTLS_SSL_SERVER_CHANGE_CIPHER_SPEC) {
            if (ssl->handshake == NULL) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("丢弃握手之外的ChangeCipherSpec"));
                return MBEDTLS_ERR_SSL_UNEXPECTED_RECORD;
            }

            MBEDTLS_SSL_DEBUG_MSG(1, ("收到顺序错误的ChangeCipherSpec - 记录"));
            return MBEDTLS_ERR_SSL_EARLY_MESSAGE;
        }
#endif
    }
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT) {
        if (ssl->in_msglen != 2) {
            /* 注意：标准允许在一个消息中打包多个2字节的警报，
               但Mbed TLS当前不支持这一点。 */
            MBEDTLS_SSL_DEBUG_MSG(1, ("无效的警报消息，长度: %" MBEDTLS_PRINTF_SIZET,
                                      ssl->in_msglen));
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }

        MBEDTLS_SSL_DEBUG_MSG(2, ("收到一个警报消息，类型: [%u:%u]",
                                  ssl->in_msg[0], ssl->in_msg[1]));

        /*
         * 忽略非致命警报，除了关闭通知和不允许重新协商
         */
        if (ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_FATAL) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("这是一个致命的警报消息 (消息 %d)",
                                      ssl->in_msg[1]));
        ...
        if (ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_WARNING &&
            ...
            MBEDTLS_SSL_DEBUG_MSG(2, ("这是一个关闭通知消息"));
        ...
        if (ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_WARNING &&
            ...
            MBEDTLS_SSL_DEBUG_MSG(2, ("这是一个SSLv3不允许重新协商的警报"));
            /* 将在尝试解析ServerHello时处理 */
            return 0;
        }
#endif
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 &&
            ...
            MBEDTLS_SSL_DEBUG_MSG(2, ("这是一个SSLv3无证书的警报"));
            /* 将在mbedtls_ssl_parse_certificate()中处理 */
            return 0;
        }
#endif /* MBEDTLS_SSL_PROTO_SSL3 && MBEDTLS_SSL_SRV_C */
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_APPLICATION_DATA &&
            ...
            MBEDTLS_SSL_DEBUG_MSG(1, ("丢弃意外的应用数据"));
            return MBEDTLS_ERR_SSL_NON_FATAL;
        }
