 ...
 * 解析SSL证书
 *
 * 该函数负责解析来自对对等方的证书链，验证其有效性，并根据需要缓存证书信息
 * 它处理证书链的接收、解析和验证过程，并根据配置决定是否保持对等方的证书
 *
 * @param ssl SSL上下文指针，包含所有SSL相关数据和设置
 * @return 返回0表示成功，非0表示失败，并根据具体错误情况返回相应的错误码
 */
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    //os_printf("%s  111\n", __func__); // 打印调试信息
