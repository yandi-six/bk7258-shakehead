...
{
    //os_printf("ssl_handshake_client_step\n");
    int ret = 0;
    ...
    switch (ssl->state) {
        ...
        case MBEDTLS_SSL_CERTIFICATE_REQUEST:
            ret = ssl_parse_certificate_request(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_SERVER_HELLO_DONE:
            ret = ssl_parse_server_hello_done(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;
        ...
        case MBEDTLS_SSL_CLIENT_CERTIFICATE:
            ret = mbedtls_ssl_write_certificate(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_CLIENT_KEY_EXCHANGE:
            ret = ssl_write_client_key_exchange(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_CERTIFICATE_VERIFY:
            ret = ssl_write_certificate_verify(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_CLIENT_CHANGE_CIPHER_SPEC:
            ret = mbedtls_ssl_write_change_cipher_spec(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_CLIENT_FINISHED:
            ret = mbedtls_ssl_write_finished(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;
        ...
        case MBEDTLS_SSL_SERVER_NEW_SESSION_TICKET:
            ret = ssl_parse_new_session_ticket(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;
#endif

        case MBEDTLS_SSL_SERVER_CHANGE_CIPHER_SPEC:
            ret = mbedtls_ssl_parse_change_cipher_spec(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_SERVER_FINISHED:
            ret = mbedtls_ssl_parse_finished(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        case MBEDTLS_SSL_FLUSH_BUFFERS:
            MBEDTLS_SSL_DEBUG_MSG(2, ("handshake: done"));//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            ssl->state = MBEDTLS_SSL_HANDSHAKE_WRAPUP;
            break;

        case MBEDTLS_SSL_HANDSHAKE_WRAPUP:
            mbedtls_ssl_handshake_wrapup(ssl);//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            break;

        default:
            MBEDTLS_SSL_DEBUG_MSG(1, ("invalid state %d", ssl->state));//os_printf("ret3:%d ,ssl->state:%d\n",ret,ssl->state);
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
    //os_printf("ret end:%d ,ssl->state:%d\n",ret,ssl->state);
    return ret;
}
