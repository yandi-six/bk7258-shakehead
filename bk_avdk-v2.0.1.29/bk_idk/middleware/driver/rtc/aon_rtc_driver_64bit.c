...
extern bk_err_t bk_flash_partition_read(bk_partition_t partition, uint8_t *out_buffer, uint32_t offset, uint32_t buffer_len);
extern bk_err_t bk_flash_partition_write(bk_partition_t partition, const uint8_t *buffer, uint32_t offset, uint32_t buffer_len);

static bk_err_t aon_rtc_bake_timeofday()
{
	struct timeval rtc_keep_time = {0, 0};
	rtc_keep_time.tv_sec = s_boot_time_us/1000000;//int64_t
	rtc_keep_time.tv_usec = s_boot_time_us%1000000;
	//bake up current time to flash
	
#if 0//CONFIG_EASY_FLASH
	...
	for(int key_index = 0; key_index < AONRTC_DEEPSLEEP_KEEPTIME_EF_KEYNUM; key_index++)
	{
		...
		AON_RTC_LOGD("%s, sec=%u, usec=%u, ret=%d.\n",__func__, (uint32_t)rtc_keep_time.tv_sec, (uint32_t)rtc_keep_time.tv_usec, ret);
	}
#else
	//AON_RTC_LOGI("TODO:Please save the time to none lost memory\r\n", __func__);
	//bk_printf("%s, sec=%d, usec=%d.\n",__func__, (uint32_t)rtc_keep_time.tv_sec, (uint32_t)rtc_keep_time.tv_usec);
	char sec[10] = {0};
	char usec[10] = {0};
	char timeSave[20] = {0};
	snprintf(sec, sizeof(sec), "%ld", (long)rtc_keep_time.tv_sec);
    snprintf(usec, sizeof(usec), "%ld", (long)rtc_keep_time.tv_usec);
	snprintf(timeSave, sizeof(timeSave), "%s%s",sec, usec);
	bk_printf("%s, sec=%s, usec=%s.\n", __func__, sec, usec);

	bk_err_t ret = BK_OK;

	ret = bk_flash_partition_erase(BK_PARTITION_EASYFLASH, 4096, sizeof(timeSave));
	if (ret == BK_OK)
	{
		bk_flash_partition_write(BK_PARTITION_EASYFLASH, (const uint8_t *)&timeSave, 4096, sizeof(timeSave));
	}
...
static void aon_rtc_get_timeofday(struct timeval *time_p)
{
	...
	char timeSave[20] = {0};
	long temp;
	int re  = bk_flash_partition_read(BK_PARTITION_EASYFLASH, (uint8_t *)&timeSave, 4096, sizeof(timeSave));
	if (re == BK_OK){
		bk_printf("time:%s\n",timeSave);
		//memcpy(sec,timeSave,16);
		time_p->tv_sec = strtol(sec, NULL, sizeof(sec));
		if(sscanf(timeSave,"%10d%10d",&time_p->tv_sec,&time_p->tv_usec) == 2)
			bk_printf("%s tv_sec:%d tv_usec:%d\n",__func__, (uint32_t)time_p->tv_sec, (uint32_t)time_p->tv_usec);
		else
			bk_printf("%s else tv_sec:%d tv_usec:%d\n",__func__, (uint32_t)time_p->tv_sec, (uint32_t)time_p->tv_usec);
	}else
		bk_printf("%s wrong \n",__func__);
