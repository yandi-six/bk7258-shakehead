...
#if 0
bk_err_t scale0_hal_set_firstaddr(uint32_t addr)
{
	scale0_ll_set_0x04_first_addr(addr);
	
	return BK_OK;
}

bk_err_t scale0_hal_set_dest_addr(uint32_t addr)
{
	scale0_ll_set_0x0c_dest_addr(addr)
	return BK_OK;
}
#endif
...
bk_err_t  scale0_hal_set_pingpong_line_mode(scale_mode_t format)
{
	if (format == DVP_H264_SCALE)
		scale0_ll_set_0x05_line_index(1);
	else
		scale0_ll_set_0x05_line_index(0);
	return BK_OK;
}

bk_err_t scale0_hal_clear_int_status(void)
{
	scale0_ll_set_0x0f_clr_int_stat(1);
...
{
	return scale0_ll_get_0x11_int_stat();
}
...
bk_err_t scale0_hal_data_format(pixel_format_t fmt)
{
	switch (fmt)
	{
		case PIXEL_FMT_RGB565:
			scale0_ll_set_0x09_format(0);
			break;
		case PIXEL_FMT_YUYV:
			scale0_ll_set_0x09_format(1);
			break;
		case PIXEL_FMT_RGB888:
			scale0_ll_set_0x09_format(2);
			break;
		default:
			break;
	}
...
{
	scale0_ll_set_0x0b_pic_width(src_width);
	scale0_ll_set_0x0d_out_pic_width(dst_width);
	return BK_OK;
}
...
bk_err_t scale0_hal_set_row_coef_loc_params(uint16_t dst_width, uint16_t *params)
{
	...
	{
		*((volatile unsigned long *)(SCALE0_LL_REG_BASE + (0x20 + i)*4)) = params[i];
	}
