...
static void adc_task_main(void *arg)
{
	...
	BK_LOG_ON_ERR(bk_adc_set_channel(adc_config->chan));
	...
	BK_LOG_ON_ERR(bk_adc_deinit(adc_config->chan));
...
int adc_start_app1(void)
{
	...
	s_adc_task1_config.adc_config.chan = ADC_DETECT_CHANNEL1;
...
int adc_start_app2(void)
{
	...
	s_adc_task2_config.adc_config.chan = ADC_DETECT_CHANNEL2;
